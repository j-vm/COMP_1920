int[32] registers; 
memset(memory,0,1024);
int[] flags; 
 int PC = 0; 
 int main(){

PC = PC + 80;
registers[13] = registers[0] + registers[2536];
registers[2] = registers[0] + registers[1224];
//[ERROR]: Couldn't understand instruction
registers[1] = registers[0] + registers[2552];
PC = PC + 200;
registers[15] = PC;
registers[0]=registers[0]||registers[0];
registers[1] = registers[1] + -20;
memory[10] = registers[15];
PC = PC + 388;
registers[15] = PC;
registers[0]=registers[0]||registers[0];
//[ERROR]: Couldn't understand instruction
registers[0]=registers[0]||registers[0];
PC = PC + 824;
registers[15] = PC;
registers[0]=registers[0]||registers[0];
registers[1] = registers[1] + registers[-8];
memory[01] = registers[15];
PC = PC + -932;
registers[15] = PC;
registers[0]=registers[0]||registers[0];
//[ERROR]: Couldn't understand instruction
registers[3] = registers[0] + registers[0];
registers[1] = registers[1] + registers[-28];
registers[5] = registers[0] + registers[2524];
registers[6] = registers[0] + registers[2540];
if (registers[3] == 0 ) { PC = PC + 16; }
memory[10] = registers[15];
registers[3] = memory[02528];
//[ERROR]: Couldn't understand instruction
registers[4] = registers[0] + registers[0];
if (registers[3] == 0 ) { PC = PC + 28; }
registers[15] = memory[10];
//[ERROR]: Couldn't understand instruction
registers[1] = registers[1] + registers[28];
PC = PC + -80;
registers[15] = PC;
registers[0]=registers[0]||registers[0];
registers[3] = memory[01200];
registers[1] = registers[1] + registers[-32];
memory[128] = registers[19];
registers[19] = registers[0] + registers[1200];
registers[18] = registers[3] ^ -1;
if (registers[18] == 0 ) { PC = PC + 28; }
memory[10] = registers[15];
registers[15] = memory[10];
registers[19] = memory[128];
//[ERROR]: Couldn't understand instruction
registers[1] = registers[1] + registers[32];
registers[15] = memory[01];
//[ERROR]: Couldn't understand instruction
registers[1] = registers[1] + registers[8];
registers[6] = registers[0] + 0;
registers[7] = registers[0] + 0;
PC = PC + 284;
registers[15] = PC;
registers[5] = registers[0] + 0;
registers[5] = registers[0] + registers[1252];
registers[6] = registers[0] + registers[1236];
registers[1] = registers[1] + registers[-1052];
registers[7] = registers[0] + registers[256];
registers[8] = registers[0] + registers[4];
memory[10] = registers[15];
PC = PC + -264;
registers[15] = PC;
registers[9] = registers[1] + registers[28];
registers[1] = registers[1] + registers[-12];
memory[14] = registers[19];
memory[18] = registers[22];
registers[4] = memory[50];
registers[3] = memory[60];
registers[19] = registers[9] + registers[0];
registers[11] = registers[6] + registers[0];
registers[3] = registers[3] * registers[4];
registers[10] = registers[5] + registers[0];
registers[22] = registers[7] + registers[0];
memory[190] = registers[3];
registers[4] = memory[60];
registers[3] = memory[114];
registers[6] = memory[54];
registers[5] = memory[50];
registers[9] = registers[8] + registers[0];
registers[4] = registers[4] * registers[6];
registers[3] = registers[3] * registers[5];
registers[4] = registers[4] + registers[3];
memory[194] = registers[4];
registers[3] = memory[110];
registers[8] = memory[108];
registers[5] = memory[114];
registers[7] = memory[104];
registers[4] = memory[118];
registers[6] = memory[100];
registers[3] = registers[3] * registers[8];
registers[5] = registers[5] * registers[7];
registers[4] = registers[4] * registers[6];
registers[3] = registers[3] + registers[5];
registers[3] = registers[3] + registers[4];
registers[18] = registers[0] + registers[3];
//[ERROR]: Couldn't understand instruction
if (registers[18] >= 0 ) { PC = PC + 92; }
memory[198] = registers[3];
registers[6] = registers[10] + registers[12];
registers[12] = registers[0] + registers[3];
if (registers[9] <= 0 ) { PC = PC + 52; }
[[wile(CONDITION){]]
registers[10] = registers[0] + registers[0];
registers[8] = registers[6] + registers[0];
registers[7] = registers[10] + registers[0];
registers[3]= ( registers[12] << 1026;
[[wile(CONDITION){]]
registers[12] = registers[12] + registers[1];
memory[193] = registers[10];
registers[18]=registers[22] - registers[12]1;
if (registers[18] != 0 ) { PC = PC + -68; }
registers[6] = registers[6] + registers[4];
if (registers[9] <= 0 ) { PC = PC + 52; }
[[wile(CONDITION){]]
registers[10] = registers[0] + registers[0];
registers[8] = registers[6] + registers[0];
registers[7] = registers[10] + registers[0];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[12] << 1026;
[[wile(CONDITION){]]
registers[12] = registers[12] + registers[1];
memory[193] = registers[10];
registers[18]=registers[22] - registers[12]1;
if (registers[18] != 0 ) { PC = PC + -68; }
registers[6] = registers[6] + registers[4];
if (registers[9] <= 0 ) { PC = PC + 52; }
[[wile(CONDITION){]]
registers[10] = registers[0] + registers[0];
registers[8] = registers[6] + registers[0];
registers[7] = registers[10] + registers[0];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[12] << 1026;
[[wile(CONDITION){]]
registers[12] = registers[12] + registers[1];
memory[193] = registers[10];
registers[18]=registers[22] - registers[12]1;
if (registers[18] != 0 ) { PC = PC + -68; }
registers[6] = registers[6] + registers[4];
if (registers[9] <= 0 ) { PC = PC + 52; }
[[wile(CONDITION){]]
registers[10] = registers[0] + registers[0];
registers[8] = registers[6] + registers[0];
registers[7] = registers[10] + registers[0];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[12] << 1026;
[[wile(CONDITION){]]
registers[12] = registers[12] + registers[1];
memory[193] = registers[10];
registers[18]=registers[22] - registers[12]1;
if (registers[18] != 0 ) { PC = PC + -68; }
registers[6] = registers[6] + registers[4];
if (registers[9] <= 0 ) { PC = PC + 52; }
[[wile(CONDITION){]]
registers[10] = registers[0] + registers[0];
registers[8] = registers[6] + registers[0];
registers[7] = registers[10] + registers[0];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[12] << 1026;
[[wile(CONDITION){]]
registers[12] = registers[12] + registers[1];
memory[193] = registers[10];
registers[18]=registers[22] - registers[12]1;
if (registers[18] != 0 ) { PC = PC + -68; }
registers[6] = registers[6] + registers[4];
if (registers[9] <= 0 ) { PC = PC + 52; }
[[wile(CONDITION){]]
registers[10] = registers[0] + registers[0];
registers[8] = registers[6] + registers[0];
registers[7] = registers[10] + registers[0];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[12] << 1026;
[[wile(CONDITION){]]
registers[12] = registers[12] + registers[1];
memory[193] = registers[10];
registers[18]=registers[22] - registers[12]1;
if (registers[18] != 0 ) { PC = PC + -68; }
registers[6] = registers[6] + registers[4];
if (registers[9] <= 0 ) { PC = PC + 52; }
[[wile(CONDITION){]]
registers[10] = registers[0] + registers[0];
registers[8] = registers[6] + registers[0];
registers[7] = registers[10] + registers[0];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[12] << 1026;
[[wile(CONDITION){]]
registers[12] = registers[12] + registers[1];
memory[193] = registers[10];
registers[18]=registers[22] - registers[12]1;
if (registers[18] != 0 ) { PC = PC + -68; }
registers[6] = registers[6] + registers[4];
if (registers[9] <= 0 ) { PC = PC + 52; }
[[wile(CONDITION){]]
registers[10] = registers[0] + registers[0];
registers[8] = registers[6] + registers[0];
registers[7] = registers[10] + registers[0];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[12] << 1026;
[[wile(CONDITION){]]
registers[12] = registers[12] + registers[1];
memory[193] = registers[10];
registers[18]=registers[22] - registers[12]1;
if (registers[18] != 0 ) { PC = PC + -68; }
registers[6] = registers[6] + registers[4];
if (registers[9] <= 0 ) { PC = PC + 52; }
[[wile(CONDITION){]]
registers[10] = registers[0] + registers[0];
registers[8] = registers[6] + registers[0];
registers[7] = registers[10] + registers[0];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[12] << 1026;
[[wile(CONDITION){]]
registers[12] = registers[12] + registers[1];
memory[193] = registers[10];
registers[18]=registers[22] - registers[12]1;
if (registers[18] != 0 ) { PC = PC + -68; }
registers[6] = registers[6] + registers[4];
if (registers[9] <= 0 ) { PC = PC + 52; }
[[wile(CONDITION){]]
registers[10] = registers[0] + registers[0];
registers[8] = registers[6] + registers[0];
registers[7] = registers[10] + registers[0];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[12] << 1026;
[[wile(CONDITION){]]
registers[12] = registers[12] + registers[1];
memory[193] = registers[10];
registers[18]=registers[22] - registers[12]1;
if (registers[18] != 0 ) { PC = PC + -68; }
registers[6] = registers[6] + registers[4];
if (registers[9] <= 0 ) { PC = PC + 52; }
[[wile(CONDITION){]]
registers[10] = registers[0] + registers[0];
registers[8] = registers[6] + registers[0];
registers[7] = registers[10] + registers[0];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[12] << 1026;
[[wile(CONDITION){]]
registers[12] = registers[12] + registers[1];
memory[193] = registers[10];
registers[18]=registers[22] - registers[12]1;
if (registers[18] != 0 ) { PC = PC + -68; }
registers[6] = registers[6] + registers[4];
if (registers[9] <= 0 ) { PC = PC + 52; }
[[wile(CONDITION){]]
registers[10] = registers[0] + registers[0];
registers[8] = registers[6] + registers[0];
registers[7] = registers[10] + registers[0];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[12] << 1026;
[[wile(CONDITION){]]
registers[12] = registers[12] + registers[1];
memory[193] = registers[10];
registers[18]=registers[22] - registers[12]1;
if (registers[18] != 0 ) { PC = PC + -68; }
registers[6] = registers[6] + registers[4];
if (registers[9] <= 0 ) { PC = PC + 52; }
[[wile(CONDITION){]]
registers[10] = registers[0] + registers[0];
registers[8] = registers[6] + registers[0];
registers[7] = registers[10] + registers[0];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[12] << 1026;
[[wile(CONDITION){]]
registers[12] = registers[12] + registers[1];
memory[193] = registers[10];
registers[18]=registers[22] - registers[12]1;
if (registers[18] != 0 ) { PC = PC + -68; }
registers[6] = registers[6] + registers[4];
if (registers[9] <= 0 ) { PC = PC + 52; }
[[wile(CONDITION){]]
registers[10] = registers[0] + registers[0];
registers[8] = registers[6] + registers[0];
registers[7] = registers[10] + registers[0];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[12] << 1026;
[[wile(CONDITION){]]
registers[12] = registers[12] + registers[1];
memory[193] = registers[10];
registers[18]=registers[22] - registers[12]1;
if (registers[18] != 0 ) { PC = PC + -68; }
registers[6] = registers[6] + registers[4];
if (registers[9] <= 0 ) { PC = PC + 52; }
[[wile(CONDITION){]]
registers[10] = registers[0] + registers[0];
registers[8] = registers[6] + registers[0];
registers[7] = registers[10] + registers[0];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[12] << 1026;
[[wile(CONDITION){]]
registers[12] = registers[12] + registers[1];
memory[193] = registers[10];
registers[18]=registers[22] - registers[12]1;
if (registers[18] != 0 ) { PC = PC + -68; }
registers[6] = registers[6] + registers[4];
if (registers[9] <= 0 ) { PC = PC + 52; }
[[wile(CONDITION){]]
registers[10] = registers[0] + registers[0];
registers[8] = registers[6] + registers[0];
registers[7] = registers[10] + registers[0];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[12] << 1026;
[[wile(CONDITION){]]
registers[12] = registers[12] + registers[1];
memory[193] = registers[10];
registers[18]=registers[22] - registers[12]1;
if (registers[18] != 0 ) { PC = PC + -68; }
registers[6] = registers[6] + registers[4];
if (registers[9] <= 0 ) { PC = PC + 52; }
[[wile(CONDITION){]]
registers[10] = registers[0] + registers[0];
registers[8] = registers[6] + registers[0];
registers[7] = registers[10] + registers[0];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[12] << 1026;
[[wile(CONDITION){]]
registers[12] = registers[12] + registers[1];
memory[193] = registers[10];
registers[18]=registers[22] - registers[12]1;
if (registers[18] != 0 ) { PC = PC + -68; }
registers[6] = registers[6] + registers[4];
if (registers[9] <= 0 ) { PC = PC + 52; }
[[wile(CONDITION){]]
registers[10] = registers[0] + registers[0];
registers[8] = registers[6] + registers[0];
registers[7] = registers[10] + registers[0];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[12] << 1026;
[[wile(CONDITION){]]
registers[12] = registers[12] + registers[1];
memory[193] = registers[10];
registers[18]=registers[22] - registers[12]1;
if (registers[18] != 0 ) { PC = PC + -68; }
registers[6] = registers[6] + registers[4];
if (registers[9] <= 0 ) { PC = PC + 52; }
[[wile(CONDITION){]]
registers[10] = registers[0] + registers[0];
registers[8] = registers[6] + registers[0];
registers[7] = registers[10] + registers[0];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[12] << 1026;
[[wile(CONDITION){]]
registers[12] = registers[12] + registers[1];
memory[193] = registers[10];
registers[18]=registers[22] - registers[12]1;
if (registers[18] != 0 ) { PC = PC + -68; }
registers[6] = registers[6] + registers[4];
if (registers[9] <= 0 ) { PC = PC + 52; }
[[wile(CONDITION){]]
registers[10] = registers[0] + registers[0];
registers[8] = registers[6] + registers[0];
registers[7] = registers[10] + registers[0];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[12] << 1026;
[[wile(CONDITION){]]
registers[12] = registers[12] + registers[1];
memory[193] = registers[10];
registers[18]=registers[22] - registers[12]1;
if (registers[18] != 0 ) { PC = PC + -68; }
registers[6] = registers[6] + registers[4];
if (registers[9] <= 0 ) { PC = PC + 52; }
[[wile(CONDITION){]]
registers[10] = registers[0] + registers[0];
registers[8] = registers[6] + registers[0];
registers[7] = registers[10] + registers[0];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[12] << 1026;
[[wile(CONDITION){]]
registers[12] = registers[12] + registers[1];
memory[193] = registers[10];
registers[18]=registers[22] - registers[12]1;
if (registers[18] != 0 ) { PC = PC + -68; }
registers[6] = registers[6] + registers[4];
if (registers[9] <= 0 ) { PC = PC + 52; }
[[wile(CONDITION){]]
registers[10] = registers[0] + registers[0];
registers[8] = registers[6] + registers[0];
registers[7] = registers[10] + registers[0];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[12] << 1026;
[[wile(CONDITION){]]
registers[12] = registers[12] + registers[1];
memory[193] = registers[10];
registers[18]=registers[22] - registers[12]1;
if (registers[18] != 0 ) { PC = PC + -68; }
registers[6] = registers[6] + registers[4];
if (registers[9] <= 0 ) { PC = PC + 52; }
[[wile(CONDITION){]]
registers[10] = registers[0] + registers[0];
registers[8] = registers[6] + registers[0];
registers[7] = registers[10] + registers[0];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[12] << 1026;
[[wile(CONDITION){]]
registers[12] = registers[12] + registers[1];
memory[193] = registers[10];
registers[18]=registers[22] - registers[12]1;
if (registers[18] != 0 ) { PC = PC + -68; }
registers[6] = registers[6] + registers[4];
if (registers[9] <= 0 ) { PC = PC + 52; }
[[wile(CONDITION){]]
registers[10] = registers[0] + registers[0];
registers[8] = registers[6] + registers[0];
registers[7] = registers[10] + registers[0];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[12] << 1026;
[[wile(CONDITION){]]
registers[12] = registers[12] + registers[1];
memory[193] = registers[10];
registers[18]=registers[22] - registers[12]1;
if (registers[18] != 0 ) { PC = PC + -68; }
registers[6] = registers[6] + registers[4];
if (registers[9] <= 0 ) { PC = PC + 52; }
[[wile(CONDITION){]]
registers[10] = registers[0] + registers[0];
registers[8] = registers[6] + registers[0];
registers[7] = registers[10] + registers[0];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[12] << 1026;
[[wile(CONDITION){]]
registers[12] = registers[12] + registers[1];
memory[193] = registers[10];
registers[18]=registers[22] - registers[12]1;
if (registers[18] != 0 ) { PC = PC + -68; }
registers[6] = registers[6] + registers[4];
if (registers[9] <= 0 ) { PC = PC + 52; }
[[wile(CONDITION){]]
registers[10] = registers[0] + registers[0];
registers[8] = registers[6] + registers[0];
registers[7] = registers[10] + registers[0];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[12] << 1026;
[[wile(CONDITION){]]
registers[12] = registers[12] + registers[1];
memory[193] = registers[10];
registers[18]=registers[22] - registers[12]1;
if (registers[18] != 0 ) { PC = PC + -68; }
registers[6] = registers[6] + registers[4];
if (registers[9] <= 0 ) { PC = PC + 52; }
[[wile(CONDITION){]]
registers[10] = registers[0] + registers[0];
registers[8] = registers[6] + registers[0];
registers[7] = registers[10] + registers[0];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[12] << 1026;
[[wile(CONDITION){]]
registers[12] = registers[12] + registers[1];
memory[193] = registers[10];
registers[18]=registers[22] - registers[12]1;
if (registers[18] != 0 ) { PC = PC + -68; }
registers[6] = registers[6] + registers[4];
if (registers[9] <= 0 ) { PC = PC + 52; }
[[wile(CONDITION){]]
registers[10] = registers[0] + registers[0];
registers[8] = registers[6] + registers[0];
registers[7] = registers[10] + registers[0];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[12] << 1026;
[[wile(CONDITION){]]
registers[12] = registers[12] + registers[1];
memory[193] = registers[10];
registers[18]=registers[22] - registers[12]1;
if (registers[18] != 0 ) { PC = PC + -68; }
registers[6] = registers[6] + registers[4];
if (registers[9] <= 0 ) { PC = PC + 52; }
[[wile(CONDITION){]]
registers[10] = registers[0] + registers[0];
registers[8] = registers[6] + registers[0];
registers[7] = registers[10] + registers[0];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[12] << 1026;
[[wile(CONDITION){]]
registers[12] = registers[12] + registers[1];
memory[193] = registers[10];
registers[18]=registers[22] - registers[12]1;
if (registers[18] != 0 ) { PC = PC + -68; }
registers[6] = registers[6] + registers[4];
if (registers[9] <= 0 ) { PC = PC + 52; }
[[wile(CONDITION){]]
registers[10] = registers[0] + registers[0];
registers[8] = registers[6] + registers[0];
registers[7] = registers[10] + registers[0];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[12] << 1026;
[[wile(CONDITION){]]
registers[12] = registers[12] + registers[1];
memory[193] = registers[10];
registers[18]=registers[22] - registers[12]1;
if (registers[18] != 0 ) { PC = PC + -68; }
registers[6] = registers[6] + registers[4];
if (registers[9] <= 0 ) { PC = PC + 52; }
[[wile(CONDITION){]]
registers[10] = registers[0] + registers[0];
registers[8] = registers[6] + registers[0];
registers[7] = registers[10] + registers[0];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[12] << 1026;
[[wile(CONDITION){]]
registers[12] = registers[12] + registers[1];
memory[193] = registers[10];
registers[18]=registers[22] - registers[12]1;
if (registers[18] != 0 ) { PC = PC + -68; }
registers[6] = registers[6] + registers[4];
if (registers[9] <= 0 ) { PC = PC + 52; }
[[wile(CONDITION){]]
registers[10] = registers[0] + registers[0];
registers[8] = registers[6] + registers[0];
registers[7] = registers[10] + registers[0];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[12] << 1026;
[[wile(CONDITION){]]
registers[12] = registers[12] + registers[1];
memory[193] = registers[10];
registers[18]=registers[22] - registers[12]1;
if (registers[18] != 0 ) { PC = PC + -68; }
registers[6] = registers[6] + registers[4];
if (registers[9] <= 0 ) { PC = PC + 52; }
[[wile(CONDITION){]]
registers[10] = registers[0] + registers[0];
registers[8] = registers[6] + registers[0];
registers[7] = registers[10] + registers[0];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[12] << 1026;
[[wile(CONDITION){]]
registers[12] = registers[12] + registers[1];
memory[193] = registers[10];
registers[18]=registers[22] - registers[12]1;
if (registers[18] != 0 ) { PC = PC + -68; }
registers[6] = registers[6] + registers[4];
if (registers[9] <= 0 ) { PC = PC + 52; }
[[wile(CONDITION){]]
registers[10] = registers[0] + registers[0];
registers[8] = registers[6] + registers[0];
registers[7] = registers[10] + registers[0];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[12] << 1026;
[[wile(CONDITION){]]
registers[12] = registers[12] + registers[1];
memory[193] = registers[10];
registers[18]=registers[22] - registers[12]1;
if (registers[18] != 0 ) { PC = PC + -68; }
registers[6] = registers[6] + registers[4];
if (registers[9] <= 0 ) { PC = PC + 52; }
[[wile(CONDITION){]]
registers[10] = registers[0] + registers[0];
registers[8] = registers[6] + registers[0];
registers[7] = registers[10] + registers[0];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[12] << 1026;
[[wile(CONDITION){]]
registers[12] = registers[12] + registers[1];
memory[193] = registers[10];
registers[18]=registers[22] - registers[12]1;
if (registers[18] != 0 ) { PC = PC + -68; }
registers[6] = registers[6] + registers[4];
if (registers[9] <= 0 ) { PC = PC + 52; }
[[wile(CONDITION){]]
registers[10] = registers[0] + registers[0];
registers[8] = registers[6] + registers[0];
registers[7] = registers[10] + registers[0];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[12] << 1026;
[[wile(CONDITION){]]
registers[12] = registers[12] + registers[1];
memory[193] = registers[10];
registers[18]=registers[22] - registers[12]1;
if (registers[18] != 0 ) { PC = PC + -68; }
registers[6] = registers[6] + registers[4];
if (registers[9] <= 0 ) { PC = PC + 52; }
[[wile(CONDITION){]]
registers[10] = registers[0] + registers[0];
registers[8] = registers[6] + registers[0];
registers[7] = registers[10] + registers[0];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[12] << 1026;
[[wile(CONDITION){]]
registers[12] = registers[12] + registers[1];
memory[193] = registers[10];
registers[18]=registers[22] - registers[12]1;
if (registers[18] != 0 ) { PC = PC + -68; }
registers[6] = registers[6] + registers[4];
if (registers[9] <= 0 ) { PC = PC + 52; }
[[wile(CONDITION){]]
registers[10] = registers[0] + registers[0];
registers[8] = registers[6] + registers[0];
registers[7] = registers[10] + registers[0];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[12] << 1026;
[[wile(CONDITION){]]
registers[12] = registers[12] + registers[1];
memory[193] = registers[10];
registers[18]=registers[22] - registers[12]1;
if (registers[18] != 0 ) { PC = PC + -68; }
registers[6] = registers[6] + registers[4];
if (registers[9] <= 0 ) { PC = PC + 52; }
[[wile(CONDITION){]]
registers[10] = registers[0] + registers[0];
registers[8] = registers[6] + registers[0];
registers[7] = registers[10] + registers[0];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[12] << 1026;
[[wile(CONDITION){]]
registers[12] = registers[12] + registers[1];
memory[193] = registers[10];
registers[18]=registers[22] - registers[12]1;
if (registers[18] != 0 ) { PC = PC + -68; }
registers[6] = registers[6] + registers[4];
if (registers[9] <= 0 ) { PC = PC + 52; }
[[wile(CONDITION){]]
registers[10] = registers[0] + registers[0];
registers[8] = registers[6] + registers[0];
registers[7] = registers[10] + registers[0];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[12] << 1026;
[[wile(CONDITION){]]
registers[12] = registers[12] + registers[1];
memory[193] = registers[10];
registers[18]=registers[22] - registers[12]1;
if (registers[18] != 0 ) { PC = PC + -68; }
registers[6] = registers[6] + registers[4];
if (registers[9] <= 0 ) { PC = PC + 52; }
[[wile(CONDITION){]]
registers[10] = registers[0] + registers[0];
registers[8] = registers[6] + registers[0];
registers[7] = registers[10] + registers[0];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[12] << 1026;
[[wile(CONDITION){]]
registers[12] = registers[12] + registers[1];
memory[193] = registers[10];
registers[18]=registers[22] - registers[12]1;
if (registers[18] != 0 ) { PC = PC + -68; }
registers[6] = registers[6] + registers[4];
if (registers[9] <= 0 ) { PC = PC + 52; }
[[wile(CONDITION){]]
registers[10] = registers[0] + registers[0];
registers[8] = registers[6] + registers[0];
registers[7] = registers[10] + registers[0];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[12] << 1026;
[[wile(CONDITION){]]
registers[12] = registers[12] + registers[1];
memory[193] = registers[10];
registers[18]=registers[22] - registers[12]1;
if (registers[18] != 0 ) { PC = PC + -68; }
registers[6] = registers[6] + registers[4];
if (registers[9] <= 0 ) { PC = PC + 52; }
[[wile(CONDITION){]]
registers[10] = registers[0] + registers[0];
registers[8] = registers[6] + registers[0];
registers[7] = registers[10] + registers[0];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[12] << 1026;
[[wile(CONDITION){]]
registers[12] = registers[12] + registers[1];
memory[193] = registers[10];
registers[18]=registers[22] - registers[12]1;
if (registers[18] != 0 ) { PC = PC + -68; }
registers[6] = registers[6] + registers[4];
if (registers[9] <= 0 ) { PC = PC + 52; }
[[wile(CONDITION){]]
registers[10] = registers[0] + registers[0];
registers[8] = registers[6] + registers[0];
registers[7] = registers[10] + registers[0];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[12] << 1026;
[[wile(CONDITION){]]
registers[12] = registers[12] + registers[1];
memory[193] = registers[10];
registers[18]=registers[22] - registers[12]1;
if (registers[18] != 0 ) { PC = PC + -68; }
registers[6] = registers[6] + registers[4];
if (registers[9] <= 0 ) { PC = PC + 52; }
[[wile(CONDITION){]]
registers[10] = registers[0] + registers[0];
registers[8] = registers[6] + registers[0];
registers[7] = registers[10] + registers[0];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[12] << 1026;
[[wile(CONDITION){]]
registers[12] = registers[12] + registers[1];
memory[193] = registers[10];
registers[18]=registers[22] - registers[12]1;
if (registers[18] != 0 ) { PC = PC + -68; }
registers[6] = registers[6] + registers[4];
if (registers[9] <= 0 ) { PC = PC + 52; }
[[wile(CONDITION){]]
registers[10] = registers[0] + registers[0];
registers[8] = registers[6] + registers[0];
registers[7] = registers[10] + registers[0];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[12] << 1026;
[[wile(CONDITION){]]
registers[12] = registers[12] + registers[1];
memory[193] = registers[10];
registers[18]=registers[22] - registers[12]1;
if (registers[18] != 0 ) { PC = PC + -68; }
registers[6] = registers[6] + registers[4];
if (registers[9] <= 0 ) { PC = PC + 52; }
[[wile(CONDITION){]]
registers[10] = registers[0] + registers[0];
registers[8] = registers[6] + registers[0];
registers[7] = registers[10] + registers[0];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[12] << 1026;
[[wile(CONDITION){]]
registers[12] = registers[12] + registers[1];
memory[193] = registers[10];
registers[18]=registers[22] - registers[12]1;
if (registers[18] != 0 ) { PC = PC + -68; }
registers[6] = registers[6] + registers[4];
if (registers[9] <= 0 ) { PC = PC + 52; }
[[wile(CONDITION){]]
registers[10] = registers[0] + registers[0];
registers[8] = registers[6] + registers[0];
registers[7] = registers[10] + registers[0];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[12] << 1026;
[[wile(CONDITION){]]
registers[12] = registers[12] + registers[1];
memory[193] = registers[10];
registers[18]=registers[22] - registers[12]1;
if (registers[18] != 0 ) { PC = PC + -68; }
registers[6] = registers[6] + registers[4];
if (registers[9] <= 0 ) { PC = PC + 52; }
[[wile(CONDITION){]]
registers[10] = registers[0] + registers[0];
registers[8] = registers[6] + registers[0];
registers[7] = registers[10] + registers[0];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[12] << 1026;
[[wile(CONDITION){]]
registers[12] = registers[12] + registers[1];
memory[193] = registers[10];
registers[18]=registers[22] - registers[12]1;
if (registers[18] != 0 ) { PC = PC + -68; }
registers[6] = registers[6] + registers[4];
if (registers[9] <= 0 ) { PC = PC + 52; }
[[wile(CONDITION){]]
registers[10] = registers[0] + registers[0];
registers[8] = registers[6] + registers[0];
registers[7] = registers[10] + registers[0];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[12] << 1026;
[[wile(CONDITION){]]
registers[12] = registers[12] + registers[1];
memory[193] = registers[10];
registers[18]=registers[22] - registers[12]1;
if (registers[18] != 0 ) { PC = PC + -68; }
registers[6] = registers[6] + registers[4];
if (registers[9] <= 0 ) { PC = PC + 52; }
[[wile(CONDITION){]]
registers[10] = registers[0] + registers[0];
registers[8] = registers[6] + registers[0];
registers[7] = registers[10] + registers[0];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[12] << 1026;
[[wile(CONDITION){]]
registers[12] = registers[12] + registers[1];
memory[193] = registers[10];
registers[18]=registers[22] - registers[12]1;
if (registers[18] != 0 ) { PC = PC + -68; }
registers[6] = registers[6] + registers[4];
if (registers[9] <= 0 ) { PC = PC + 52; }
[[wile(CONDITION){]]
registers[10] = registers[0] + registers[0];
registers[8] = registers[6] + registers[0];
registers[7] = registers[10] + registers[0];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[12] << 1026;
[[wile(CONDITION){]]
registers[12] = registers[12] + registers[1];
memory[193] = registers[10];
registers[18]=registers[22] - registers[12]1;
if (registers[18] != 0 ) { PC = PC + -68; }
registers[6] = registers[6] + registers[4];
if (registers[9] <= 0 ) { PC = PC + 52; }
[[wile(CONDITION){]]
registers[10] = registers[0] + registers[0];
registers[8] = registers[6] + registers[0];
registers[7] = registers[10] + registers[0];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[12] << 1026;
[[wile(CONDITION){]]
registers[12] = registers[12] + registers[1];
memory[193] = registers[10];
registers[18]=registers[22] - registers[12]1;
if (registers[18] != 0 ) { PC = PC + -68; }
registers[6] = registers[6] + registers[4];
if (registers[9] <= 0 ) { PC = PC + 52; }
[[wile(CONDITION){]]
registers[10] = registers[0] + registers[0];
registers[8] = registers[6] + registers[0];
registers[7] = registers[10] + registers[0];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[12] << 1026;
[[wile(CONDITION){]]
registers[12] = registers[12] + registers[1];
memory[193] = registers[10];
registers[18]=registers[22] - registers[12]1;
if (registers[18] != 0 ) { PC = PC + -68; }
registers[6] = registers[6] + registers[4];
if (registers[9] <= 0 ) { PC = PC + 52; }
[[wile(CONDITION){]]
registers[10] = registers[0] + registers[0];
registers[8] = registers[6] + registers[0];
registers[7] = registers[10] + registers[0];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[12] << 1026;
[[wile(CONDITION){]]
registers[12] = registers[12] + registers[1];
memory[193] = registers[10];
registers[18]=registers[22] - registers[12]1;
if (registers[18] != 0 ) { PC = PC + -68; }
registers[6] = registers[6] + registers[4];
if (registers[9] <= 0 ) { PC = PC + 52; }
[[wile(CONDITION){]]
registers[10] = registers[0] + registers[0];
registers[8] = registers[6] + registers[0];
registers[7] = registers[10] + registers[0];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[12] << 1026;
[[wile(CONDITION){]]
registers[12] = registers[12] + registers[1];
memory[193] = registers[10];
registers[18]=registers[22] - registers[12]1;
if (registers[18] != 0 ) { PC = PC + -68; }
registers[6] = registers[6] + registers[4];
if (registers[9] <= 0 ) { PC = PC + 52; }
[[wile(CONDITION){]]
registers[10] = registers[0] + registers[0];
registers[8] = registers[6] + registers[0];
registers[7] = registers[10] + registers[0];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[12] << 1026;
[[wile(CONDITION){]]
registers[12] = registers[12] + registers[1];
memory[193] = registers[10];
registers[18]=registers[22] - registers[12]1;
if (registers[18] != 0 ) { PC = PC + -68; }
registers[6] = registers[6] + registers[4];
if (registers[9] <= 0 ) { PC = PC + 52; }
[[wile(CONDITION){]]
registers[10] = registers[0] + registers[0];
registers[8] = registers[6] + registers[0];
registers[7] = registers[10] + registers[0];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[12] << 1026;
[[wile(CONDITION){]]
registers[12] = registers[12] + registers[1];
memory[193] = registers[10];
registers[18]=registers[22] - registers[12]1;
if (registers[18] != 0 ) { PC = PC + -68; }
registers[6] = registers[6] + registers[4];
if (registers[9] <= 0 ) { PC = PC + 52; }
[[wile(CONDITION){]]
registers[10] = registers[0] + registers[0];
registers[8] = registers[6] + registers[0];
registers[7] = registers[10] + registers[0];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[12] << 1026;
[[wile(CONDITION){]]
registers[12] = registers[12] + registers[1];
memory[193] = registers[10];
registers[18]=registers[22] - registers[12]1;
if (registers[18] != 0 ) { PC = PC + -68; }
registers[6] = registers[6] + registers[4];
if (registers[9] <= 0 ) { PC = PC + 52; }
[[wile(CONDITION){]]
registers[10] = registers[0] + registers[0];
registers[8] = registers[6] + registers[0];
registers[7] = registers[10] + registers[0];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[12] << 1026;
[[wile(CONDITION){]]
registers[12] = registers[12] + registers[1];
memory[193] = registers[10];
registers[18]=registers[22] - registers[12]1;
if (registers[18] != 0 ) { PC = PC + -68; }
registers[6] = registers[6] + registers[4];
if (registers[9] <= 0 ) { PC = PC + 52; }
[[wile(CONDITION){]]
registers[10] = registers[0] + registers[0];
registers[8] = registers[6] + registers[0];
registers[7] = registers[10] + registers[0];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[12] << 1026;
[[wile(CONDITION){]]
registers[12] = registers[12] + registers[1];
memory[193] = registers[10];
registers[18]=registers[22] - registers[12]1;
if (registers[18] != 0 ) { PC = PC + -68; }
registers[6] = registers[6] + registers[4];
if (registers[9] <= 0 ) { PC = PC + 52; }
[[wile(CONDITION){]]
registers[10] = registers[0] + registers[0];
registers[8] = registers[6] + registers[0];
registers[7] = registers[10] + registers[0];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[12] << 1026;
[[wile(CONDITION){]]
registers[12] = registers[12] + registers[1];
memory[193] = registers[10];
registers[18]=registers[22] - registers[12]1;
if (registers[18] != 0 ) { PC = PC + -68; }
registers[6] = registers[6] + registers[4];
if (registers[9] <= 0 ) { PC = PC + 52; }
[[wile(CONDITION){]]
registers[10] = registers[0] + registers[0];
registers[8] = registers[6] + registers[0];
registers[7] = registers[10] + registers[0];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[12] << 1026;
[[wile(CONDITION){]]
registers[12] = registers[12] + registers[1];
memory[193] = registers[10];
registers[18]=registers[22] - registers[12]1;
if (registers[18] != 0 ) { PC = PC + -68; }
registers[6] = registers[6] + registers[4];
if (registers[9] <= 0 ) { PC = PC + 52; }
[[wile(CONDITION){]]
registers[10] = registers[0] + registers[0];
registers[8] = registers[6] + registers[0];
registers[7] = registers[10] + registers[0];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[12] << 1026;
[[wile(CONDITION){]]
registers[12] = registers[12] + registers[1];
memory[193] = registers[10];
registers[18]=registers[22] - registers[12]1;
if (registers[18] != 0 ) { PC = PC + -68; }
registers[6] = registers[6] + registers[4];
if (registers[9] <= 0 ) { PC = PC + 52; }
[[wile(CONDITION){]]
registers[10] = registers[0] + registers[0];
registers[8] = registers[6] + registers[0];
registers[7] = registers[10] + registers[0];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[12] << 1026;
[[wile(CONDITION){]]
registers[12] = registers[12] + registers[1];
memory[193] = registers[10];
registers[18]=registers[22] - registers[12]1;
if (registers[18] != 0 ) { PC = PC + -68; }
registers[6] = registers[6] + registers[4];
if (registers[9] <= 0 ) { PC = PC + 52; }
[[wile(CONDITION){]]
registers[10] = registers[0] + registers[0];
registers[8] = registers[6] + registers[0];
registers[7] = registers[10] + registers[0];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[12] << 1026;
[[wile(CONDITION){]]
registers[12] = registers[12] + registers[1];
memory[193] = registers[10];
registers[18]=registers[22] - registers[12]1;
if (registers[18] != 0 ) { PC = PC + -68; }
registers[6] = registers[6] + registers[4];
if (registers[9] <= 0 ) { PC = PC + 52; }
[[wile(CONDITION){]]
registers[10] = registers[0] + registers[0];
registers[8] = registers[6] + registers[0];
registers[7] = registers[10] + registers[0];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[12] << 1026;
[[wile(CONDITION){]]
registers[12] = registers[12] + registers[1];
memory[193] = registers[10];
registers[18]=registers[22] - registers[12]1;
if (registers[18] != 0 ) { PC = PC + -68; }
registers[6] = registers[6] + registers[4];
if (registers[9] <= 0 ) { PC = PC + 52; }
[[wile(CONDITION){]]
registers[10] = registers[0] + registers[0];
registers[8] = registers[6] + registers[0];
registers[7] = registers[10] + registers[0];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[12] << 1026;
[[wile(CONDITION){]]
registers[12] = registers[12] + registers[1];
memory[193] = registers[10];
registers[18]=registers[22] - registers[12]1;
if (registers[18] != 0 ) { PC = PC + -68; }
registers[6] = registers[6] + registers[4];
if (registers[9] <= 0 ) { PC = PC + 52; }
[[wile(CONDITION){]]
registers[10] = registers[0] + registers[0];
registers[8] = registers[6] + registers[0];
registers[7] = registers[10] + registers[0];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[12] << 1026;
[[wile(CONDITION){]]
registers[12] = registers[12] + registers[1];
memory[193] = registers[10];
registers[18]=registers[22] - registers[12]1;
if (registers[18] != 0 ) { PC = PC + -68; }
registers[6] = registers[6] + registers[4];
if (registers[9] <= 0 ) { PC = PC + 52; }
[[wile(CONDITION){]]
registers[10] = registers[0] + registers[0];
registers[8] = registers[6] + registers[0];
registers[7] = registers[10] + registers[0];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[12] << 1026;
[[wile(CONDITION){]]
registers[12] = registers[12] + registers[1];
memory[193] = registers[10];
registers[18]=registers[22] - registers[12]1;
if (registers[18] != 0 ) { PC = PC + -68; }
registers[6] = registers[6] + registers[4];
if (registers[9] <= 0 ) { PC = PC + 52; }
[[wile(CONDITION){]]
registers[10] = registers[0] + registers[0];
registers[8] = registers[6] + registers[0];
registers[7] = registers[10] + registers[0];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[12] << 1026;
[[wile(CONDITION){]]
registers[12] = registers[12] + registers[1];
memory[193] = registers[10];
registers[18]=registers[22] - registers[12]1;
if (registers[18] != 0 ) { PC = PC + -68; }
registers[6] = registers[6] + registers[4];
if (registers[9] <= 0 ) { PC = PC + 52; }
[[wile(CONDITION){]]
registers[10] = registers[0] + registers[0];
registers[8] = registers[6] + registers[0];
registers[7] = registers[10] + registers[0];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[12] << 1026;
[[wile(CONDITION){]]
registers[12] = registers[12] + registers[1];
memory[193] = registers[10];
registers[18]=registers[22] - registers[12]1;
if (registers[18] != 0 ) { PC = PC + -68; }
registers[6] = registers[6] + registers[4];
if (registers[9] <= 0 ) { PC = PC + 52; }
[[wile(CONDITION){]]
registers[10] = registers[0] + registers[0];
registers[8] = registers[6] + registers[0];
registers[7] = registers[10] + registers[0];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[12] << 1026;
[[wile(CONDITION){]]
registers[12] = registers[12] + registers[1];
memory[193] = registers[10];
registers[18]=registers[22] - registers[12]1;
if (registers[18] != 0 ) { PC = PC + -68; }
registers[6] = registers[6] + registers[4];
if (registers[9] <= 0 ) { PC = PC + 52; }
[[wile(CONDITION){]]
registers[10] = registers[0] + registers[0];
registers[8] = registers[6] + registers[0];
registers[7] = registers[10] + registers[0];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[12] << 1026;
[[wile(CONDITION){]]
registers[12] = registers[12] + registers[1];
memory[193] = registers[10];
registers[18]=registers[22] - registers[12]1;
if (registers[18] != 0 ) { PC = PC + -68; }
registers[6] = registers[6] + registers[4];
if (registers[9] <= 0 ) { PC = PC + 52; }
[[wile(CONDITION){]]
registers[10] = registers[0] + registers[0];
registers[8] = registers[6] + registers[0];
registers[7] = registers[10] + registers[0];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[12] << 1026;
[[wile(CONDITION){]]
registers[12] = registers[12] + registers[1];
memory[193] = registers[10];
registers[18]=registers[22] - registers[12]1;
if (registers[18] != 0 ) { PC = PC + -68; }
registers[6] = registers[6] + registers[4];
if (registers[9] <= 0 ) { PC = PC + 52; }
[[wile(CONDITION){]]
registers[10] = registers[0] + registers[0];
registers[8] = registers[6] + registers[0];
registers[7] = registers[10] + registers[0];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[12] << 1026;
[[wile(CONDITION){]]
registers[12] = registers[12] + registers[1];
memory[193] = registers[10];
registers[18]=registers[22] - registers[12]1;
if (registers[18] != 0 ) { PC = PC + -68; }
registers[6] = registers[6] + registers[4];
if (registers[9] <= 0 ) { PC = PC + 52; }
[[wile(CONDITION){]]
registers[10] = registers[0] + registers[0];
registers[8] = registers[6] + registers[0];
registers[7] = registers[10] + registers[0];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[12] << 1026;
[[wile(CONDITION){]]
registers[12] = registers[12] + registers[1];
memory[193] = registers[10];
registers[18]=registers[22] - registers[12]1;
if (registers[18] != 0 ) { PC = PC + -68; }
registers[6] = registers[6] + registers[4];
if (registers[9] <= 0 ) { PC = PC + 52; }
[[wile(CONDITION){]]
registers[10] = registers[0] + registers[0];
registers[8] = registers[6] + registers[0];
registers[7] = registers[10] + registers[0];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[12] << 1026;
[[wile(CONDITION){]]
registers[12] = registers[12] + registers[1];
memory[193] = registers[10];
registers[18]=registers[22] - registers[12]1;
if (registers[18] != 0 ) { PC = PC + -68; }
registers[6] = registers[6] + registers[4];
if (registers[9] <= 0 ) { PC = PC + 52; }
[[wile(CONDITION){]]
registers[10] = registers[0] + registers[0];
registers[8] = registers[6] + registers[0];
registers[7] = registers[10] + registers[0];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[12] << 1026;
[[wile(CONDITION){]]
registers[12] = registers[12] + registers[1];
memory[193] = registers[10];
registers[18]=registers[22] - registers[12]1;
if (registers[18] != 0 ) { PC = PC + -68; }
registers[6] = registers[6] + registers[4];
if (registers[9] <= 0 ) { PC = PC + 52; }
[[wile(CONDITION){]]
registers[10] = registers[0] + registers[0];
registers[8] = registers[6] + registers[0];
registers[7] = registers[10] + registers[0];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[12] << 1026;
[[wile(CONDITION){]]
registers[12] = registers[12] + registers[1];
memory[193] = registers[10];
registers[18]=registers[22] - registers[12]1;
if (registers[18] != 0 ) { PC = PC + -68; }
registers[6] = registers[6] + registers[4];
if (registers[9] <= 0 ) { PC = PC + 52; }
[[wile(CONDITION){]]
registers[10] = registers[0] + registers[0];
registers[8] = registers[6] + registers[0];
registers[7] = registers[10] + registers[0];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[12] << 1026;
[[wile(CONDITION){]]
registers[12] = registers[12] + registers[1];
memory[193] = registers[10];
registers[18]=registers[22] - registers[12]1;
if (registers[18] != 0 ) { PC = PC + -68; }
registers[6] = registers[6] + registers[4];
if (registers[9] <= 0 ) { PC = PC + 52; }
[[wile(CONDITION){]]
registers[10] = registers[0] + registers[0];
registers[8] = registers[6] + registers[0];
registers[7] = registers[10] + registers[0];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[12] << 1026;
[[wile(CONDITION){]]
registers[12] = registers[12] + registers[1];
memory[193] = registers[10];
registers[18]=registers[22] - registers[12]1;
if (registers[18] != 0 ) { PC = PC + -68; }
registers[6] = registers[6] + registers[4];
if (registers[9] <= 0 ) { PC = PC + 52; }
[[wile(CONDITION){]]
registers[10] = registers[0] + registers[0];
registers[8] = registers[6] + registers[0];
registers[7] = registers[10] + registers[0];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[12] << 1026;
[[wile(CONDITION){]]
registers[12] = registers[12] + registers[1];
memory[193] = registers[10];
registers[18]=registers[22] - registers[12]1;
if (registers[18] != 0 ) { PC = PC + -68; }
registers[6] = registers[6] + registers[4];
if (registers[9] <= 0 ) { PC = PC + 52; }
[[wile(CONDITION){]]
registers[10] = registers[0] + registers[0];
registers[8] = registers[6] + registers[0];
registers[7] = registers[10] + registers[0];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[12] << 1026;
[[wile(CONDITION){]]
registers[12] = registers[12] + registers[1];
memory[193] = registers[10];
registers[18]=registers[22] - registers[12]1;
if (registers[18] != 0 ) { PC = PC + -68; }
registers[6] = registers[6] + registers[4];
if (registers[9] <= 0 ) { PC = PC + 52; }
[[wile(CONDITION){]]
registers[10] = registers[0] + registers[0];
registers[8] = registers[6] + registers[0];
registers[7] = registers[10] + registers[0];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[12] << 1026;
[[wile(CONDITION){]]
registers[12] = registers[12] + registers[1];
memory[193] = registers[10];
registers[18]=registers[22] - registers[12]1;
if (registers[18] != 0 ) { PC = PC + -68; }
registers[6] = registers[6] + registers[4];
if (registers[9] <= 0 ) { PC = PC + 52; }
[[wile(CONDITION){]]
registers[10] = registers[0] + registers[0];
registers[8] = registers[6] + registers[0];
registers[7] = registers[10] + registers[0];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[12] << 1026;
[[wile(CONDITION){]]
registers[12] = registers[12] + registers[1];
memory[193] = registers[10];
registers[18]=registers[22] - registers[12]1;
if (registers[18] != 0 ) { PC = PC + -68; }
registers[6] = registers[6] + registers[4];
if (registers[9] <= 0 ) { PC = PC + 52; }
[[wile(CONDITION){]]
registers[10] = registers[0] + registers[0];
registers[8] = registers[6] + registers[0];
registers[7] = registers[10] + registers[0];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[12] << 1026;
[[wile(CONDITION){]]
registers[12] = registers[12] + registers[1];
memory[193] = registers[10];
registers[18]=registers[22] - registers[12]1;
if (registers[18] != 0 ) { PC = PC + -68; }
registers[6] = registers[6] + registers[4];
if (registers[9] <= 0 ) { PC = PC + 52; }
[[wile(CONDITION){]]
registers[10] = registers[0] + registers[0];
registers[8] = registers[6] + registers[0];
registers[7] = registers[10] + registers[0];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[12] << 1026;
[[wile(CONDITION){]]
registers[12] = registers[12] + registers[1];
memory[193] = registers[10];
registers[18]=registers[22] - registers[12]1;
if (registers[18] != 0 ) { PC = PC + -68; }
registers[6] = registers[6] + registers[4];
if (registers[9] <= 0 ) { PC = PC + 52; }
[[wile(CONDITION){]]
registers[10] = registers[0] + registers[0];
registers[8] = registers[6] + registers[0];
registers[7] = registers[10] + registers[0];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[12] << 1026;
[[wile(CONDITION){]]
registers[12] = registers[12] + registers[1];
memory[193] = registers[10];
registers[18]=registers[22] - registers[12]1;
if (registers[18] != 0 ) { PC = PC + -68; }
registers[6] = registers[6] + registers[4];
if (registers[9] <= 0 ) { PC = PC + 52; }
[[wile(CONDITION){]]
registers[10] = registers[0] + registers[0];
registers[8] = registers[6] + registers[0];
registers[7] = registers[10] + registers[0];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[12] << 1026;
[[wile(CONDITION){]]
registers[12] = registers[12] + registers[1];
memory[193] = registers[10];
registers[18]=registers[22] - registers[12]1;
if (registers[18] != 0 ) { PC = PC + -68; }
registers[6] = registers[6] + registers[4];
if (registers[9] <= 0 ) { PC = PC + 52; }
[[wile(CONDITION){]]
registers[10] = registers[0] + registers[0];
registers[8] = registers[6] + registers[0];
registers[7] = registers[10] + registers[0];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[12] << 1026;
[[wile(CONDITION){]]
registers[12] = registers[12] + registers[1];
memory[193] = registers[10];
registers[18]=registers[22] - registers[12]1;
if (registers[18] != 0 ) { PC = PC + -68; }
registers[6] = registers[6] + registers[4];
if (registers[9] <= 0 ) { PC = PC + 52; }
[[wile(CONDITION){]]
registers[10] = registers[0] + registers[0];
registers[8] = registers[6] + registers[0];
registers[7] = registers[10] + registers[0];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[12] << 1026;
[[wile(CONDITION){]]
registers[12] = registers[12] + registers[1];
memory[193] = registers[10];
registers[18]=registers[22] - registers[12]1;
if (registers[18] != 0 ) { PC = PC + -68; }
registers[6] = registers[6] + registers[4];
if (registers[9] <= 0 ) { PC = PC + 52; }
[[wile(CONDITION){]]
registers[10] = registers[0] + registers[0];
registers[8] = registers[6] + registers[0];
registers[7] = registers[10] + registers[0];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[12] << 1026;
[[wile(CONDITION){]]
registers[12] = registers[12] + registers[1];
memory[193] = registers[10];
registers[18]=registers[22] - registers[12]1;
if (registers[18] != 0 ) { PC = PC + -68; }
registers[6] = registers[6] + registers[4];
if (registers[9] <= 0 ) { PC = PC + 52; }
[[wile(CONDITION){]]
registers[10] = registers[0] + registers[0];
registers[8] = registers[6] + registers[0];
registers[7] = registers[10] + registers[0];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[12] << 1026;
[[wile(CONDITION){]]
registers[12] = registers[12] + registers[1];
memory[193] = registers[10];
registers[18]=registers[22] - registers[12]1;
if (registers[18] != 0 ) { PC = PC + -68; }
registers[6] = registers[6] + registers[4];
if (registers[9] <= 0 ) { PC = PC + 52; }
[[wile(CONDITION){]]
registers[10] = registers[0] + registers[0];
registers[8] = registers[6] + registers[0];
registers[7] = registers[10] + registers[0];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[12] << 1026;
[[wile(CONDITION){]]
registers[12] = registers[12] + registers[1];
memory[193] = registers[10];
registers[18]=registers[22] - registers[12]1;
if (registers[18] != 0 ) { PC = PC + -68; }
registers[6] = registers[6] + registers[4];
if (registers[9] <= 0 ) { PC = PC + 52; }
[[wile(CONDITION){]]
registers[10] = registers[0] + registers[0];
registers[8] = registers[6] + registers[0];
registers[7] = registers[10] + registers[0];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[12] << 1026;
[[wile(CONDITION){]]
registers[12] = registers[12] + registers[1];
memory[193] = registers[10];
registers[18]=registers[22] - registers[12]1;
if (registers[18] != 0 ) { PC = PC + -68; }
registers[6] = registers[6] + registers[4];
if (registers[9] <= 0 ) { PC = PC + 52; }
[[wile(CONDITION){]]
registers[10] = registers[0] + registers[0];
registers[8] = registers[6] + registers[0];
registers[7] = registers[10] + registers[0];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[12] << 1026;
[[wile(CONDITION){]]
registers[12] = registers[12] + registers[1];
memory[193] = registers[10];
registers[18]=registers[22] - registers[12]1;
if (registers[18] != 0 ) { PC = PC + -68; }
registers[6] = registers[6] + registers[4];
if (registers[9] <= 0 ) { PC = PC + 52; }
[[wile(CONDITION){]]
registers[10] = registers[0] + registers[0];
registers[8] = registers[6] + registers[0];
registers[7] = registers[10] + registers[0];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[12] << 1026;
[[wile(CONDITION){]]
registers[12] = registers[12] + registers[1];
memory[193] = registers[10];
registers[18]=registers[22] - registers[12]1;
if (registers[18] != 0 ) { PC = PC + -68; }
registers[6] = registers[6] + registers[4];
if (registers[9] <= 0 ) { PC = PC + 52; }
[[wile(CONDITION){]]
registers[10] = registers[0] + registers[0];
registers[8] = registers[6] + registers[0];
registers[7] = registers[10] + registers[0];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[12] << 1026;
[[wile(CONDITION){]]
registers[12] = registers[12] + registers[1];
memory[193] = registers[10];
registers[18]=registers[22] - registers[12]1;
if (registers[18] != 0 ) { PC = PC + -68; }
registers[6] = registers[6] + registers[4];
if (registers[9] <= 0 ) { PC = PC + 52; }
[[wile(CONDITION){]]
registers[10] = registers[0] + registers[0];
registers[8] = registers[6] + registers[0];
registers[7] = registers[10] + registers[0];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[12] << 1026;
[[wile(CONDITION){]]
registers[12] = registers[12] + registers[1];
memory[193] = registers[10];
registers[18]=registers[22] - registers[12]1;
if (registers[18] != 0 ) { PC = PC + -68; }
registers[6] = registers[6] + registers[4];
if (registers[9] <= 0 ) { PC = PC + 52; }
[[wile(CONDITION){]]
registers[10] = registers[0] + registers[0];
registers[8] = registers[6] + registers[0];
registers[7] = registers[10] + registers[0];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[12] << 1026;
[[wile(CONDITION){]]
registers[12] = registers[12] + registers[1];
memory[193] = registers[10];
registers[18]=registers[22] - registers[12]1;
if (registers[18] != 0 ) { PC = PC + -68; }
registers[6] = registers[6] + registers[4];
if (registers[9] <= 0 ) { PC = PC + 52; }
[[wile(CONDITION){]]
registers[10] = registers[0] + registers[0];
registers[8] = registers[6] + registers[0];
registers[7] = registers[10] + registers[0];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[12] << 1026;
[[wile(CONDITION){]]
registers[12] = registers[12] + registers[1];
memory[193] = registers[10];
registers[18]=registers[22] - registers[12]1;
if (registers[18] != 0 ) { PC = PC + -68; }
registers[6] = registers[6] + registers[4];
if (registers[9] <= 0 ) { PC = PC + 52; }
[[wile(CONDITION){]]
registers[10] = registers[0] + registers[0];
registers[8] = registers[6] + registers[0];
registers[7] = registers[10] + registers[0];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[12] << 1026;
[[wile(CONDITION){]]
registers[12] = registers[12] + registers[1];
memory[193] = registers[10];
registers[18]=registers[22] - registers[12]1;
if (registers[18] != 0 ) { PC = PC + -68; }
registers[6] = registers[6] + registers[4];
if (registers[9] <= 0 ) { PC = PC + 52; }
[[wile(CONDITION){]]
registers[10] = registers[0] + registers[0];
registers[8] = registers[6] + registers[0];
registers[7] = registers[10] + registers[0];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[12] << 1026;
[[wile(CONDITION){]]
registers[12] = registers[12] + registers[1];
memory[193] = registers[10];
registers[18]=registers[22] - registers[12]1;
if (registers[18] != 0 ) { PC = PC + -68; }
registers[6] = registers[6] + registers[4];
if (registers[9] <= 0 ) { PC = PC + 52; }
[[wile(CONDITION){]]
registers[10] = registers[0] + registers[0];
registers[8] = registers[6] + registers[0];
registers[7] = registers[10] + registers[0];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[12] << 1026;
[[wile(CONDITION){]]
registers[12] = registers[12] + registers[1];
memory[193] = registers[10];
registers[18]=registers[22] - registers[12]1;
if (registers[18] != 0 ) { PC = PC + -68; }
registers[6] = registers[6] + registers[4];
if (registers[9] <= 0 ) { PC = PC + 52; }
[[wile(CONDITION){]]
registers[10] = registers[0] + registers[0];
registers[8] = registers[6] + registers[0];
registers[7] = registers[10] + registers[0];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[12] << 1026;
[[wile(CONDITION){]]
registers[12] = registers[12] + registers[1];
memory[193] = registers[10];
registers[18]=registers[22] - registers[12]1;
if (registers[18] != 0 ) { PC = PC + -68; }
registers[6] = registers[6] + registers[4];
if (registers[9] <= 0 ) { PC = PC + 52; }
[[wile(CONDITION){]]
registers[10] = registers[0] + registers[0];
registers[8] = registers[6] + registers[0];
registers[7] = registers[10] + registers[0];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[12] << 1026;
[[wile(CONDITION){]]
registers[12] = registers[12] + registers[1];
memory[193] = registers[10];
registers[18]=registers[22] - registers[12]1;
if (registers[18] != 0 ) { PC = PC + -68; }
registers[6] = registers[6] + registers[4];
if (registers[9] <= 0 ) { PC = PC + 52; }
[[wile(CONDITION){]]
registers[10] = registers[0] + registers[0];
registers[8] = registers[6] + registers[0];
registers[7] = registers[10] + registers[0];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[12] << 1026;
[[wile(CONDITION){]]
registers[12] = registers[12] + registers[1];
memory[193] = registers[10];
registers[18]=registers[22] - registers[12]1;
if (registers[18] != 0 ) { PC = PC + -68; }
registers[6] = registers[6] + registers[4];
if (registers[9] <= 0 ) { PC = PC + 52; }
[[wile(CONDITION){]]
registers[10] = registers[0] + registers[0];
registers[8] = registers[6] + registers[0];
registers[7] = registers[10] + registers[0];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[12] << 1026;
[[wile(CONDITION){]]
registers[12] = registers[12] + registers[1];
memory[193] = registers[10];
registers[18]=registers[22] - registers[12]1;
if (registers[18] != 0 ) { PC = PC + -68; }
registers[6] = registers[6] + registers[4];
if (registers[9] <= 0 ) { PC = PC + 52; }
[[wile(CONDITION){]]
registers[10] = registers[0] + registers[0];
registers[8] = registers[6] + registers[0];
registers[7] = registers[10] + registers[0];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[12] << 1026;
[[wile(CONDITION){]]
registers[12] = registers[12] + registers[1];
memory[193] = registers[10];
registers[18]=registers[22] - registers[12]1;
if (registers[18] != 0 ) { PC = PC + -68; }
registers[6] = registers[6] + registers[4];
if (registers[9] <= 0 ) { PC = PC + 52; }
[[wile(CONDITION){]]
registers[10] = registers[0] + registers[0];
registers[8] = registers[6] + registers[0];
registers[7] = registers[10] + registers[0];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[12] << 1026;
[[wile(CONDITION){]]
registers[12] = registers[12] + registers[1];
memory[193] = registers[10];
registers[18]=registers[22] - registers[12]1;
if (registers[18] != 0 ) { PC = PC + -68; }
registers[6] = registers[6] + registers[4];
if (registers[9] <= 0 ) { PC = PC + 52; }
[[wile(CONDITION){]]
registers[10] = registers[0] + registers[0];
registers[8] = registers[6] + registers[0];
registers[7] = registers[10] + registers[0];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[12] << 1026;
[[wile(CONDITION){]]
registers[12] = registers[12] + registers[1];
memory[193] = registers[10];
registers[18]=registers[22] - registers[12]1;
if (registers[18] != 0 ) { PC = PC + -68; }
registers[6] = registers[6] + registers[4];
if (registers[9] <= 0 ) { PC = PC + 52; }
[[wile(CONDITION){]]
registers[10] = registers[0] + registers[0];
registers[8] = registers[6] + registers[0];
registers[7] = registers[10] + registers[0];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[12] << 1026;
[[wile(CONDITION){]]
registers[12] = registers[12] + registers[1];
memory[193] = registers[10];
registers[18]=registers[22] - registers[12]1;
if (registers[18] != 0 ) { PC = PC + -68; }
registers[6] = registers[6] + registers[4];
if (registers[9] <= 0 ) { PC = PC + 52; }
[[wile(CONDITION){]]
registers[10] = registers[0] + registers[0];
registers[8] = registers[6] + registers[0];
registers[7] = registers[10] + registers[0];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[12] << 1026;
[[wile(CONDITION){]]
registers[12] = registers[12] + registers[1];
memory[193] = registers[10];
registers[18]=registers[22] - registers[12]1;
if (registers[18] != 0 ) { PC = PC + -68; }
registers[6] = registers[6] + registers[4];
if (registers[9] <= 0 ) { PC = PC + 52; }
[[wile(CONDITION){]]
registers[10] = registers[0] + registers[0];
registers[8] = registers[6] + registers[0];
registers[7] = registers[10] + registers[0];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[12] << 1026;
[[wile(CONDITION){]]
registers[12] = registers[12] + registers[1];
memory[193] = registers[10];
registers[18]=registers[22] - registers[12]1;
if (registers[18] != 0 ) { PC = PC + -68; }
registers[6] = registers[6] + registers[4];
if (registers[9] <= 0 ) { PC = PC + 52; }
[[wile(CONDITION){]]
registers[10] = registers[0] + registers[0];
registers[8] = registers[6] + registers[0];
registers[7] = registers[10] + registers[0];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[12] << 1026;
[[wile(CONDITION){]]
registers[12] = registers[12] + registers[1];
memory[193] = registers[10];
registers[18]=registers[22] - registers[12]1;
if (registers[18] != 0 ) { PC = PC + -68; }
registers[6] = registers[6] + registers[4];
if (registers[9] <= 0 ) { PC = PC + 52; }
[[wile(CONDITION){]]
registers[10] = registers[0] + registers[0];
registers[8] = registers[6] + registers[0];
registers[7] = registers[10] + registers[0];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[12] << 1026;
[[wile(CONDITION){]]
registers[12] = registers[12] + registers[1];
memory[193] = registers[10];
registers[18]=registers[22] - registers[12]1;
if (registers[18] != 0 ) { PC = PC + -68; }
registers[6] = registers[6] + registers[4];
if (registers[9] <= 0 ) { PC = PC + 52; }
[[wile(CONDITION){]]
registers[10] = registers[0] + registers[0];
registers[8] = registers[6] + registers[0];
registers[7] = registers[10] + registers[0];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[12] << 1026;
[[wile(CONDITION){]]
registers[12] = registers[12] + registers[1];
memory[193] = registers[10];
registers[18]=registers[22] - registers[12]1;
if (registers[18] != 0 ) { PC = PC + -68; }
registers[6] = registers[6] + registers[4];
if (registers[9] <= 0 ) { PC = PC + 52; }
[[wile(CONDITION){]]
registers[10] = registers[0] + registers[0];
registers[8] = registers[6] + registers[0];
registers[7] = registers[10] + registers[0];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[12] << 1026;
[[wile(CONDITION){]]
registers[12] = registers[12] + registers[1];
memory[193] = registers[10];
registers[18]=registers[22] - registers[12]1;
if (registers[18] != 0 ) { PC = PC + -68; }
registers[6] = registers[6] + registers[4];
if (registers[9] <= 0 ) { PC = PC + 52; }
[[wile(CONDITION){]]
registers[10] = registers[0] + registers[0];
registers[8] = registers[6] + registers[0];
registers[7] = registers[10] + registers[0];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[12] << 1026;
[[wile(CONDITION){]]
registers[12] = registers[12] + registers[1];
memory[193] = registers[10];
registers[18]=registers[22] - registers[12]1;
if (registers[18] != 0 ) { PC = PC + -68; }
registers[6] = registers[6] + registers[4];
if (registers[9] <= 0 ) { PC = PC + 52; }
[[wile(CONDITION){]]
registers[10] = registers[0] + registers[0];
registers[8] = registers[6] + registers[0];
registers[7] = registers[10] + registers[0];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[12] << 1026;
[[wile(CONDITION){]]
registers[12] = registers[12] + registers[1];
memory[193] = registers[10];
registers[18]=registers[22] - registers[12]1;
if (registers[18] != 0 ) { PC = PC + -68; }
registers[6] = registers[6] + registers[4];
if (registers[9] <= 0 ) { PC = PC + 52; }
[[wile(CONDITION){]]
registers[10] = registers[0] + registers[0];
registers[8] = registers[6] + registers[0];
registers[7] = registers[10] + registers[0];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[12] << 1026;
[[wile(CONDITION){]]
registers[12] = registers[12] + registers[1];
memory[193] = registers[10];
registers[18]=registers[22] - registers[12]1;
if (registers[18] != 0 ) { PC = PC + -68; }
registers[6] = registers[6] + registers[4];
if (registers[9] <= 0 ) { PC = PC + 52; }
[[wile(CONDITION){]]
registers[10] = registers[0] + registers[0];
registers[8] = registers[6] + registers[0];
registers[7] = registers[10] + registers[0];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[12] << 1026;
[[wile(CONDITION){]]
registers[12] = registers[12] + registers[1];
memory[193] = registers[10];
registers[18]=registers[22] - registers[12]1;
if (registers[18] != 0 ) { PC = PC + -68; }
registers[6] = registers[6] + registers[4];
if (registers[9] <= 0 ) { PC = PC + 52; }
[[wile(CONDITION){]]
registers[10] = registers[0] + registers[0];
registers[8] = registers[6] + registers[0];
registers[7] = registers[10] + registers[0];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[12] << 1026;
[[wile(CONDITION){]]
registers[12] = registers[12] + registers[1];
memory[193] = registers[10];
registers[18]=registers[22] - registers[12]1;
if (registers[18] != 0 ) { PC = PC + -68; }
registers[6] = registers[6] + registers[4];
if (registers[9] <= 0 ) { PC = PC + 52; }
[[wile(CONDITION){]]
registers[10] = registers[0] + registers[0];
registers[8] = registers[6] + registers[0];
registers[7] = registers[10] + registers[0];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[12] << 1026;
[[wile(CONDITION){]]
registers[12] = registers[12] + registers[1];
memory[193] = registers[10];
registers[18]=registers[22] - registers[12]1;
if (registers[18] != 0 ) { PC = PC + -68; }
registers[6] = registers[6] + registers[4];
if (registers[9] <= 0 ) { PC = PC + 52; }
[[wile(CONDITION){]]
registers[10] = registers[0] + registers[0];
registers[8] = registers[6] + registers[0];
registers[7] = registers[10] + registers[0];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[12] << 1026;
[[wile(CONDITION){]]
registers[12] = registers[12] + registers[1];
memory[193] = registers[10];
registers[18]=registers[22] - registers[12]1;
if (registers[18] != 0 ) { PC = PC + -68; }
registers[6] = registers[6] + registers[4];
if (registers[9] <= 0 ) { PC = PC + 52; }
[[wile(CONDITION){]]
registers[10] = registers[0] + registers[0];
registers[8] = registers[6] + registers[0];
registers[7] = registers[10] + registers[0];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[12] << 1026;
[[wile(CONDITION){]]
registers[12] = registers[12] + registers[1];
memory[193] = registers[10];
registers[18]=registers[22] - registers[12]1;
if (registers[18] != 0 ) { PC = PC + -68; }
registers[6] = registers[6] + registers[4];
if (registers[9] <= 0 ) { PC = PC + 52; }
[[wile(CONDITION){]]
registers[10] = registers[0] + registers[0];
registers[8] = registers[6] + registers[0];
registers[7] = registers[10] + registers[0];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[12] << 1026;
[[wile(CONDITION){]]
registers[12] = registers[12] + registers[1];
memory[193] = registers[10];
registers[18]=registers[22] - registers[12]1;
if (registers[18] != 0 ) { PC = PC + -68; }
registers[6] = registers[6] + registers[4];
if (registers[9] <= 0 ) { PC = PC + 52; }
[[wile(CONDITION){]]
registers[10] = registers[0] + registers[0];
registers[8] = registers[6] + registers[0];
registers[7] = registers[10] + registers[0];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[12] << 1026;
[[wile(CONDITION){]]
registers[12] = registers[12] + registers[1];
memory[193] = registers[10];
registers[18]=registers[22] - registers[12]1;
if (registers[18] != 0 ) { PC = PC + -68; }
registers[6] = registers[6] + registers[4];
if (registers[9] <= 0 ) { PC = PC + 52; }
[[wile(CONDITION){]]
registers[10] = registers[0] + registers[0];
registers[8] = registers[6] + registers[0];
registers[7] = registers[10] + registers[0];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[12] << 1026;
[[wile(CONDITION){]]
registers[12] = registers[12] + registers[1];
memory[193] = registers[10];
registers[18]=registers[22] - registers[12]1;
if (registers[18] != 0 ) { PC = PC + -68; }
registers[6] = registers[6] + registers[4];
if (registers[9] <= 0 ) { PC = PC + 52; }
[[wile(CONDITION){]]
registers[10] = registers[0] + registers[0];
registers[8] = registers[6] + registers[0];
registers[7] = registers[10] + registers[0];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[12] << 1026;
[[wile(CONDITION){]]
registers[12] = registers[12] + registers[1];
memory[193] = registers[10];
registers[18]=registers[22] - registers[12]1;
if (registers[18] != 0 ) { PC = PC + -68; }
registers[6] = registers[6] + registers[4];
if (registers[9] <= 0 ) { PC = PC + 52; }
[[wile(CONDITION){]]
registers[10] = registers[0] + registers[0];
registers[8] = registers[6] + registers[0];
registers[7] = registers[10] + registers[0];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[12] << 1026;
[[wile(CONDITION){]]
registers[12] = registers[12] + registers[1];
memory[193] = registers[10];
registers[18]=registers[22] - registers[12]1;
if (registers[18] != 0 ) { PC = PC + -68; }
registers[6] = registers[6] + registers[4];
if (registers[9] <= 0 ) { PC = PC + 52; }
[[wile(CONDITION){]]
registers[10] = registers[0] + registers[0];
registers[8] = registers[6] + registers[0];
registers[7] = registers[10] + registers[0];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[12] << 1026;
[[wile(CONDITION){]]
registers[12] = registers[12] + registers[1];
memory[193] = registers[10];
registers[18]=registers[22] - registers[12]1;
if (registers[18] != 0 ) { PC = PC + -68; }
registers[6] = registers[6] + registers[4];
if (registers[9] <= 0 ) { PC = PC + 52; }
[[wile(CONDITION){]]
registers[10] = registers[0] + registers[0];
registers[8] = registers[6] + registers[0];
registers[7] = registers[10] + registers[0];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[12] << 1026;
[[wile(CONDITION){]]
registers[12] = registers[12] + registers[1];
memory[193] = registers[10];
registers[18]=registers[22] - registers[12]1;
if (registers[18] != 0 ) { PC = PC + -68; }
registers[6] = registers[6] + registers[4];
if (registers[9] <= 0 ) { PC = PC + 52; }
[[wile(CONDITION){]]
registers[10] = registers[0] + registers[0];
registers[8] = registers[6] + registers[0];
registers[7] = registers[10] + registers[0];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[12] << 1026;
[[wile(CONDITION){]]
registers[12] = registers[12] + registers[1];
memory[193] = registers[10];
registers[18]=registers[22] - registers[12]1;
if (registers[18] != 0 ) { PC = PC + -68; }
registers[6] = registers[6] + registers[4];
if (registers[9] <= 0 ) { PC = PC + 52; }
[[wile(CONDITION){]]
registers[10] = registers[0] + registers[0];
registers[8] = registers[6] + registers[0];
registers[7] = registers[10] + registers[0];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[12] << 1026;
[[wile(CONDITION){]]
registers[12] = registers[12] + registers[1];
memory[193] = registers[10];
registers[18]=registers[22] - registers[12]1;
if (registers[18] != 0 ) { PC = PC + -68; }
registers[6] = registers[6] + registers[4];
if (registers[9] <= 0 ) { PC = PC + 52; }
[[wile(CONDITION){]]
registers[10] = registers[0] + registers[0];
registers[8] = registers[6] + registers[0];
registers[7] = registers[10] + registers[0];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[12] << 1026;
[[wile(CONDITION){]]
registers[12] = registers[12] + registers[1];
memory[193] = registers[10];
registers[18]=registers[22] - registers[12]1;
if (registers[18] != 0 ) { PC = PC + -68; }
registers[6] = registers[6] + registers[4];
if (registers[9] <= 0 ) { PC = PC + 52; }
[[wile(CONDITION){]]
registers[10] = registers[0] + registers[0];
registers[8] = registers[6] + registers[0];
registers[7] = registers[10] + registers[0];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[12] << 1026;
[[wile(CONDITION){]]
registers[12] = registers[12] + registers[1];
memory[193] = registers[10];
registers[18]=registers[22] - registers[12]1;
if (registers[18] != 0 ) { PC = PC + -68; }
registers[6] = registers[6] + registers[4];
if (registers[9] <= 0 ) { PC = PC + 52; }
[[wile(CONDITION){]]
registers[10] = registers[0] + registers[0];
registers[8] = registers[6] + registers[0];
registers[7] = registers[10] + registers[0];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[12] << 1026;
[[wile(CONDITION){]]
registers[12] = registers[12] + registers[1];
memory[193] = registers[10];
registers[18]=registers[22] - registers[12]1;
if (registers[18] != 0 ) { PC = PC + -68; }
registers[6] = registers[6] + registers[4];
if (registers[9] <= 0 ) { PC = PC + 52; }
[[wile(CONDITION){]]
registers[10] = registers[0] + registers[0];
registers[8] = registers[6] + registers[0];
registers[7] = registers[10] + registers[0];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[12] << 1026;
[[wile(CONDITION){]]
registers[12] = registers[12] + registers[1];
memory[193] = registers[10];
registers[18]=registers[22] - registers[12]1;
if (registers[18] != 0 ) { PC = PC + -68; }
registers[6] = registers[6] + registers[4];
if (registers[9] <= 0 ) { PC = PC + 52; }
[[wile(CONDITION){]]
registers[10] = registers[0] + registers[0];
registers[8] = registers[6] + registers[0];
registers[7] = registers[10] + registers[0];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[12] << 1026;
[[wile(CONDITION){]]
registers[12] = registers[12] + registers[1];
memory[193] = registers[10];
registers[18]=registers[22] - registers[12]1;
if (registers[18] != 0 ) { PC = PC + -68; }
registers[6] = registers[6] + registers[4];
if (registers[9] <= 0 ) { PC = PC + 52; }
[[wile(CONDITION){]]
registers[10] = registers[0] + registers[0];
registers[8] = registers[6] + registers[0];
registers[7] = registers[10] + registers[0];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[12] << 1026;
[[wile(CONDITION){]]
registers[12] = registers[12] + registers[1];
memory[193] = registers[10];
registers[18]=registers[22] - registers[12]1;
if (registers[18] != 0 ) { PC = PC + -68; }
registers[6] = registers[6] + registers[4];
if (registers[9] <= 0 ) { PC = PC + 52; }
[[wile(CONDITION){]]
registers[10] = registers[0] + registers[0];
registers[8] = registers[6] + registers[0];
registers[7] = registers[10] + registers[0];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[12] << 1026;
[[wile(CONDITION){]]
registers[12] = registers[12] + registers[1];
memory[193] = registers[10];
registers[18]=registers[22] - registers[12]1;
if (registers[18] != 0 ) { PC = PC + -68; }
registers[6] = registers[6] + registers[4];
if (registers[9] <= 0 ) { PC = PC + 52; }
[[wile(CONDITION){]]
registers[10] = registers[0] + registers[0];
registers[8] = registers[6] + registers[0];
registers[7] = registers[10] + registers[0];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[12] << 1026;
[[wile(CONDITION){]]
registers[12] = registers[12] + registers[1];
memory[193] = registers[10];
registers[18]=registers[22] - registers[12]1;
if (registers[18] != 0 ) { PC = PC + -68; }
registers[6] = registers[6] + registers[4];
if (registers[9] <= 0 ) { PC = PC + 52; }
[[wile(CONDITION){]]
registers[10] = registers[0] + registers[0];
registers[8] = registers[6] + registers[0];
registers[7] = registers[10] + registers[0];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[12] << 1026;
[[wile(CONDITION){]]
registers[12] = registers[12] + registers[1];
memory[193] = registers[10];
registers[18]=registers[22] - registers[12]1;
if (registers[18] != 0 ) { PC = PC + -68; }
registers[6] = registers[6] + registers[4];
if (registers[9] <= 0 ) { PC = PC + 52; }
[[wile(CONDITION){]]
registers[10] = registers[0] + registers[0];
registers[8] = registers[6] + registers[0];
registers[7] = registers[10] + registers[0];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[12] << 1026;
[[wile(CONDITION){]]
registers[12] = registers[12] + registers[1];
memory[193] = registers[10];
registers[18]=registers[22] - registers[12]1;
if (registers[18] != 0 ) { PC = PC + -68; }
registers[6] = registers[6] + registers[4];
if (registers[9] <= 0 ) { PC = PC + 52; }
[[wile(CONDITION){]]
registers[10] = registers[0] + registers[0];
registers[8] = registers[6] + registers[0];
registers[7] = registers[10] + registers[0];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[12] << 1026;
[[wile(CONDITION){]]
registers[12] = registers[12] + registers[1];
memory[193] = registers[10];
registers[18]=registers[22] - registers[12]1;
if (registers[18] != 0 ) { PC = PC + -68; }
registers[6] = registers[6] + registers[4];
if (registers[9] <= 0 ) { PC = PC + 52; }
[[wile(CONDITION){]]
registers[10] = registers[0] + registers[0];
registers[8] = registers[6] + registers[0];
registers[7] = registers[10] + registers[0];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[12] << 1026;
[[wile(CONDITION){]]
registers[12] = registers[12] + registers[1];
memory[193] = registers[10];
registers[18]=registers[22] - registers[12]1;
if (registers[18] != 0 ) { PC = PC + -68; }
registers[6] = registers[6] + registers[4];
if (registers[9] <= 0 ) { PC = PC + 52; }
[[wile(CONDITION){]]
registers[10] = registers[0] + registers[0];
registers[8] = registers[6] + registers[0];
registers[7] = registers[10] + registers[0];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[12] << 1026;
[[wile(CONDITION){]]
registers[12] = registers[12] + registers[1];
memory[193] = registers[10];
registers[18]=registers[22] - registers[12]1;
if (registers[18] != 0 ) { PC = PC + -68; }
registers[6] = registers[6] + registers[4];
if (registers[9] <= 0 ) { PC = PC + 52; }
[[wile(CONDITION){]]
registers[10] = registers[0] + registers[0];
registers[8] = registers[6] + registers[0];
registers[7] = registers[10] + registers[0];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[12] << 1026;
[[wile(CONDITION){]]
registers[12] = registers[12] + registers[1];
memory[193] = registers[10];
registers[18]=registers[22] - registers[12]1;
if (registers[18] != 0 ) { PC = PC + -68; }
registers[6] = registers[6] + registers[4];
if (registers[9] <= 0 ) { PC = PC + 52; }
[[wile(CONDITION){]]
registers[10] = registers[0] + registers[0];
registers[8] = registers[6] + registers[0];
registers[7] = registers[10] + registers[0];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[12] << 1026;
[[wile(CONDITION){]]
registers[12] = registers[12] + registers[1];
memory[193] = registers[10];
registers[18]=registers[22] - registers[12]1;
if (registers[18] != 0 ) { PC = PC + -68; }
registers[6] = registers[6] + registers[4];
if (registers[9] <= 0 ) { PC = PC + 52; }
[[wile(CONDITION){]]
registers[10] = registers[0] + registers[0];
registers[8] = registers[6] + registers[0];
registers[7] = registers[10] + registers[0];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[12] << 1026;
[[wile(CONDITION){]]
registers[12] = registers[12] + registers[1];
memory[193] = registers[10];
registers[18]=registers[22] - registers[12]1;
if (registers[18] != 0 ) { PC = PC + -68; }
registers[6] = registers[6] + registers[4];
if (registers[9] <= 0 ) { PC = PC + 52; }
[[wile(CONDITION){]]
registers[10] = registers[0] + registers[0];
registers[8] = registers[6] + registers[0];
registers[7] = registers[10] + registers[0];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[12] << 1026;
[[wile(CONDITION){]]
registers[12] = registers[12] + registers[1];
memory[193] = registers[10];
registers[18]=registers[22] - registers[12]1;
if (registers[18] != 0 ) { PC = PC + -68; }
registers[6] = registers[6] + registers[4];
if (registers[9] <= 0 ) { PC = PC + 52; }
[[wile(CONDITION){]]
registers[10] = registers[0] + registers[0];
registers[8] = registers[6] + registers[0];
registers[7] = registers[10] + registers[0];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[12] << 1026;
[[wile(CONDITION){]]
registers[12] = registers[12] + registers[1];
memory[193] = registers[10];
registers[18]=registers[22] - registers[12]1;
if (registers[18] != 0 ) { PC = PC + -68; }
registers[6] = registers[6] + registers[4];
if (registers[9] <= 0 ) { PC = PC + 52; }
[[wile(CONDITION){]]
registers[10] = registers[0] + registers[0];
registers[8] = registers[6] + registers[0];
registers[7] = registers[10] + registers[0];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[12] << 1026;
[[wile(CONDITION){]]
registers[12] = registers[12] + registers[1];
memory[193] = registers[10];
registers[18]=registers[22] - registers[12]1;
if (registers[18] != 0 ) { PC = PC + -68; }
registers[6] = registers[6] + registers[4];
if (registers[9] <= 0 ) { PC = PC + 52; }
[[wile(CONDITION){]]
registers[10] = registers[0] + registers[0];
registers[8] = registers[6] + registers[0];
registers[7] = registers[10] + registers[0];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[12] << 1026;
[[wile(CONDITION){]]
registers[12] = registers[12] + registers[1];
memory[193] = registers[10];
registers[18]=registers[22] - registers[12]1;
if (registers[18] != 0 ) { PC = PC + -68; }
registers[6] = registers[6] + registers[4];
if (registers[9] <= 0 ) { PC = PC + 52; }
[[wile(CONDITION){]]
registers[10] = registers[0] + registers[0];
registers[8] = registers[6] + registers[0];
registers[7] = registers[10] + registers[0];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[12] << 1026;
[[wile(CONDITION){]]
registers[12] = registers[12] + registers[1];
memory[193] = registers[10];
registers[18]=registers[22] - registers[12]1;
if (registers[18] != 0 ) { PC = PC + -68; }
registers[6] = registers[6] + registers[4];
if (registers[9] <= 0 ) { PC = PC + 52; }
[[wile(CONDITION){]]
registers[10] = registers[0] + registers[0];
registers[8] = registers[6] + registers[0];
registers[7] = registers[10] + registers[0];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[12] << 1026;
[[wile(CONDITION){]]
registers[12] = registers[12] + registers[1];
memory[193] = registers[10];
registers[18]=registers[22] - registers[12]1;
if (registers[18] != 0 ) { PC = PC + -68; }
registers[6] = registers[6] + registers[4];
if (registers[9] <= 0 ) { PC = PC + 52; }
[[wile(CONDITION){]]
registers[10] = registers[0] + registers[0];
registers[8] = registers[6] + registers[0];
registers[7] = registers[10] + registers[0];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[12] << 1026;
[[wile(CONDITION){]]
registers[12] = registers[12] + registers[1];
memory[193] = registers[10];
registers[18]=registers[22] - registers[12]1;
if (registers[18] != 0 ) { PC = PC + -68; }
registers[6] = registers[6] + registers[4];
if (registers[9] <= 0 ) { PC = PC + 52; }
[[wile(CONDITION){]]
registers[10] = registers[0] + registers[0];
registers[8] = registers[6] + registers[0];
registers[7] = registers[10] + registers[0];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[12] << 1026;
[[wile(CONDITION){]]
registers[12] = registers[12] + registers[1];
memory[193] = registers[10];
registers[18]=registers[22] - registers[12]1;
if (registers[18] != 0 ) { PC = PC + -68; }
registers[6] = registers[6] + registers[4];
if (registers[9] <= 0 ) { PC = PC + 52; }
[[wile(CONDITION){]]
registers[10] = registers[0] + registers[0];
registers[8] = registers[6] + registers[0];
registers[7] = registers[10] + registers[0];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[12] << 1026;
[[wile(CONDITION){]]
registers[12] = registers[12] + registers[1];
memory[193] = registers[10];
registers[18]=registers[22] - registers[12]1;
if (registers[18] != 0 ) { PC = PC + -68; }
registers[6] = registers[6] + registers[4];
if (registers[9] <= 0 ) { PC = PC + 52; }
[[wile(CONDITION){]]
registers[10] = registers[0] + registers[0];
registers[8] = registers[6] + registers[0];
registers[7] = registers[10] + registers[0];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[12] << 1026;
[[wile(CONDITION){]]
registers[12] = registers[12] + registers[1];
memory[193] = registers[10];
registers[18]=registers[22] - registers[12]1;
if (registers[18] != 0 ) { PC = PC + -68; }
registers[6] = registers[6] + registers[4];
if (registers[9] <= 0 ) { PC = PC + 52; }
[[wile(CONDITION){]]
registers[10] = registers[0] + registers[0];
registers[8] = registers[6] + registers[0];
registers[7] = registers[10] + registers[0];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[12] << 1026;
[[wile(CONDITION){]]
registers[12] = registers[12] + registers[1];
memory[193] = registers[10];
registers[18]=registers[22] - registers[12]1;
if (registers[18] != 0 ) { PC = PC + -68; }
registers[6] = registers[6] + registers[4];
if (registers[9] <= 0 ) { PC = PC + 52; }
[[wile(CONDITION){]]
registers[10] = registers[0] + registers[0];
registers[8] = registers[6] + registers[0];
registers[7] = registers[10] + registers[0];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[12] << 1026;
[[wile(CONDITION){]]
registers[12] = registers[12] + registers[1];
memory[193] = registers[10];
registers[18]=registers[22] - registers[12]1;
if (registers[18] != 0 ) { PC = PC + -68; }
registers[6] = registers[6] + registers[4];
if (registers[9] <= 0 ) { PC = PC + 52; }
[[wile(CONDITION){]]
registers[10] = registers[0] + registers[0];
registers[8] = registers[6] + registers[0];
registers[7] = registers[10] + registers[0];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[12] << 1026;
[[wile(CONDITION){]]
registers[12] = registers[12] + registers[1];
memory[193] = registers[10];
registers[18]=registers[22] - registers[12]1;
if (registers[18] != 0 ) { PC = PC + -68; }
registers[6] = registers[6] + registers[4];
if (registers[9] <= 0 ) { PC = PC + 52; }
[[wile(CONDITION){]]
registers[10] = registers[0] + registers[0];
registers[8] = registers[6] + registers[0];
registers[7] = registers[10] + registers[0];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[12] << 1026;
[[wile(CONDITION){]]
registers[12] = registers[12] + registers[1];
memory[193] = registers[10];
registers[18]=registers[22] - registers[12]1;
if (registers[18] != 0 ) { PC = PC + -68; }
registers[6] = registers[6] + registers[4];
if (registers[9] <= 0 ) { PC = PC + 52; }
[[wile(CONDITION){]]
registers[10] = registers[0] + registers[0];
registers[8] = registers[6] + registers[0];
registers[7] = registers[10] + registers[0];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[12] << 1026;
[[wile(CONDITION){]]
registers[12] = registers[12] + registers[1];
memory[193] = registers[10];
registers[18]=registers[22] - registers[12]1;
if (registers[18] != 0 ) { PC = PC + -68; }
registers[6] = registers[6] + registers[4];
if (registers[9] <= 0 ) { PC = PC + 52; }
[[wile(CONDITION){]]
registers[10] = registers[0] + registers[0];
registers[8] = registers[6] + registers[0];
registers[7] = registers[10] + registers[0];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[12] << 1026;
[[wile(CONDITION){]]
registers[12] = registers[12] + registers[1];
memory[193] = registers[10];
registers[18]=registers[22] - registers[12]1;
if (registers[18] != 0 ) { PC = PC + -68; }
registers[6] = registers[6] + registers[4];
if (registers[9] <= 0 ) { PC = PC + 52; }
[[wile(CONDITION){]]
registers[10] = registers[0] + registers[0];
registers[8] = registers[6] + registers[0];
registers[7] = registers[10] + registers[0];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[12] << 1026;
[[wile(CONDITION){]]
registers[12] = registers[12] + registers[1];
memory[193] = registers[10];
registers[18]=registers[22] - registers[12]1;
if (registers[18] != 0 ) { PC = PC + -68; }
registers[6] = registers[6] + registers[4];
if (registers[9] <= 0 ) { PC = PC + 52; }
[[wile(CONDITION){]]
registers[10] = registers[0] + registers[0];
registers[8] = registers[6] + registers[0];
registers[7] = registers[10] + registers[0];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[12] << 1026;
[[wile(CONDITION){]]
registers[12] = registers[12] + registers[1];
memory[193] = registers[10];
registers[18]=registers[22] - registers[12]1;
if (registers[18] != 0 ) { PC = PC + -68; }
registers[6] = registers[6] + registers[4];
if (registers[9] <= 0 ) { PC = PC + 52; }
[[wile(CONDITION){]]
registers[10] = registers[0] + registers[0];
registers[8] = registers[6] + registers[0];
registers[7] = registers[10] + registers[0];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[12] << 1026;
[[wile(CONDITION){]]
registers[12] = registers[12] + registers[1];
memory[193] = registers[10];
registers[18]=registers[22] - registers[12]1;
if (registers[18] != 0 ) { PC = PC + -68; }
registers[6] = registers[6] + registers[4];
if (registers[9] <= 0 ) { PC = PC + 52; }
[[wile(CONDITION){]]
registers[10] = registers[0] + registers[0];
registers[8] = registers[6] + registers[0];
registers[7] = registers[10] + registers[0];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[12] << 1026;
[[wile(CONDITION){]]
registers[12] = registers[12] + registers[1];
memory[193] = registers[10];
registers[18]=registers[22] - registers[12]1;
if (registers[18] != 0 ) { PC = PC + -68; }
registers[6] = registers[6] + registers[4];
if (registers[9] <= 0 ) { PC = PC + 52; }
[[wile(CONDITION){]]
registers[10] = registers[0] + registers[0];
registers[8] = registers[6] + registers[0];
registers[7] = registers[10] + registers[0];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[12] << 1026;
[[wile(CONDITION){]]
registers[12] = registers[12] + registers[1];
memory[193] = registers[10];
registers[18]=registers[22] - registers[12]1;
if (registers[18] != 0 ) { PC = PC + -68; }
registers[6] = registers[6] + registers[4];
if (registers[9] <= 0 ) { PC = PC + 52; }
[[wile(CONDITION){]]
registers[10] = registers[0] + registers[0];
registers[8] = registers[6] + registers[0];
registers[7] = registers[10] + registers[0];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[12] << 1026;
[[wile(CONDITION){]]
registers[12] = registers[12] + registers[1];
memory[193] = registers[10];
registers[18]=registers[22] - registers[12]1;
if (registers[18] != 0 ) { PC = PC + -68; }
registers[6] = registers[6] + registers[4];
if (registers[9] <= 0 ) { PC = PC + 52; }
[[wile(CONDITION){]]
registers[10] = registers[0] + registers[0];
registers[8] = registers[6] + registers[0];
registers[7] = registers[10] + registers[0];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[12] << 1026;
[[wile(CONDITION){]]
registers[12] = registers[12] + registers[1];
memory[193] = registers[10];
registers[18]=registers[22] - registers[12]1;
if (registers[18] != 0 ) { PC = PC + -68; }
registers[6] = registers[6] + registers[4];
if (registers[9] <= 0 ) { PC = PC + 52; }
[[wile(CONDITION){]]
registers[10] = registers[0] + registers[0];
registers[8] = registers[6] + registers[0];
registers[7] = registers[10] + registers[0];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[12] << 1026;
[[wile(CONDITION){]]
registers[12] = registers[12] + registers[1];
memory[193] = registers[10];
registers[18]=registers[22] - registers[12]1;
if (registers[18] != 0 ) { PC = PC + -68; }
registers[6] = registers[6] + registers[4];
if (registers[9] <= 0 ) { PC = PC + 52; }
[[wile(CONDITION){]]
registers[10] = registers[0] + registers[0];
registers[8] = registers[6] + registers[0];
registers[7] = registers[10] + registers[0];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[12] << 1026;
[[wile(CONDITION){]]
registers[12] = registers[12] + registers[1];
memory[193] = registers[10];
registers[18]=registers[22] - registers[12]1;
if (registers[18] != 0 ) { PC = PC + -68; }
registers[6] = registers[6] + registers[4];
if (registers[9] <= 0 ) { PC = PC + 52; }
[[wile(CONDITION){]]
registers[10] = registers[0] + registers[0];
registers[8] = registers[6] + registers[0];
registers[7] = registers[10] + registers[0];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[12] << 1026;
[[wile(CONDITION){]]
registers[12] = registers[12] + registers[1];
memory[193] = registers[10];
registers[18]=registers[22] - registers[12]1;
if (registers[18] != 0 ) { PC = PC + -68; }
registers[6] = registers[6] + registers[4];
if (registers[9] <= 0 ) { PC = PC + 52; }
[[wile(CONDITION){]]
registers[10] = registers[0] + registers[0];
registers[8] = registers[6] + registers[0];
registers[7] = registers[10] + registers[0];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[12] << 1026;
[[wile(CONDITION){]]
registers[12] = registers[12] + registers[1];
memory[193] = registers[10];
registers[18]=registers[22] - registers[12]1;
if (registers[18] != 0 ) { PC = PC + -68; }
registers[6] = registers[6] + registers[4];
if (registers[9] <= 0 ) { PC = PC + 52; }
[[wile(CONDITION){]]
registers[10] = registers[0] + registers[0];
registers[8] = registers[6] + registers[0];
registers[7] = registers[10] + registers[0];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[12] << 1026;
[[wile(CONDITION){]]
registers[12] = registers[12] + registers[1];
memory[193] = registers[10];
registers[18]=registers[22] - registers[12]1;
if (registers[18] != 0 ) { PC = PC + -68; }
registers[6] = registers[6] + registers[4];
if (registers[9] <= 0 ) { PC = PC + 52; }
[[wile(CONDITION){]]
registers[10] = registers[0] + registers[0];
registers[8] = registers[6] + registers[0];
registers[7] = registers[10] + registers[0];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[12] << 1026;
[[wile(CONDITION){]]
registers[12] = registers[12] + registers[1];
memory[193] = registers[10];
registers[18]=registers[22] - registers[12]1;
if (registers[18] != 0 ) { PC = PC + -68; }
registers[6] = registers[6] + registers[4];
if (registers[9] <= 0 ) { PC = PC + 52; }
[[wile(CONDITION){]]
registers[10] = registers[0] + registers[0];
registers[8] = registers[6] + registers[0];
registers[7] = registers[10] + registers[0];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[12] << 1026;
[[wile(CONDITION){]]
registers[12] = registers[12] + registers[1];
memory[193] = registers[10];
registers[18]=registers[22] - registers[12]1;
if (registers[18] != 0 ) { PC = PC + -68; }
registers[6] = registers[6] + registers[4];
if (registers[9] <= 0 ) { PC = PC + 52; }
[[wile(CONDITION){]]
registers[10] = registers[0] + registers[0];
registers[8] = registers[6] + registers[0];
registers[7] = registers[10] + registers[0];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[12] << 1026;
[[wile(CONDITION){]]
registers[12] = registers[12] + registers[1];
memory[193] = registers[10];
registers[18]=registers[22] - registers[12]1;
if (registers[18] != 0 ) { PC = PC + -68; }
registers[6] = registers[6] + registers[4];
if (registers[9] <= 0 ) { PC = PC + 52; }
[[wile(CONDITION){]]
registers[10] = registers[0] + registers[0];
registers[8] = registers[6] + registers[0];
registers[7] = registers[10] + registers[0];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[12] << 1026;
[[wile(CONDITION){]]
registers[12] = registers[12] + registers[1];
memory[193] = registers[10];
registers[18]=registers[22] - registers[12]1;
if (registers[18] != 0 ) { PC = PC + -68; }
registers[6] = registers[6] + registers[4];
if (registers[9] <= 0 ) { PC = PC + 52; }
[[wile(CONDITION){]]
registers[10] = registers[0] + registers[0];
registers[8] = registers[6] + registers[0];
registers[7] = registers[10] + registers[0];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[12] << 1026;
[[wile(CONDITION){]]
registers[12] = registers[12] + registers[1];
memory[193] = registers[10];
registers[18]=registers[22] - registers[12]1;
if (registers[18] != 0 ) { PC = PC + -68; }
registers[6] = registers[6] + registers[4];
if (registers[9] <= 0 ) { PC = PC + 52; }
[[wile(CONDITION){]]
registers[10] = registers[0] + registers[0];
registers[8] = registers[6] + registers[0];
registers[7] = registers[10] + registers[0];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[12] << 1026;
[[wile(CONDITION){]]
registers[12] = registers[12] + registers[1];
memory[193] = registers[10];
registers[18]=registers[22] - registers[12]1;
if (registers[18] != 0 ) { PC = PC + -68; }
registers[6] = registers[6] + registers[4];
if (registers[9] <= 0 ) { PC = PC + 52; }
[[wile(CONDITION){]]
registers[10] = registers[0] + registers[0];
registers[8] = registers[6] + registers[0];
registers[7] = registers[10] + registers[0];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[12] << 1026;
[[wile(CONDITION){]]
registers[12] = registers[12] + registers[1];
memory[193] = registers[10];
registers[18]=registers[22] - registers[12]1;
if (registers[18] != 0 ) { PC = PC + -68; }
registers[6] = registers[6] + registers[4];
if (registers[9] <= 0 ) { PC = PC + 52; }
[[wile(CONDITION){]]
registers[10] = registers[0] + registers[0];
registers[8] = registers[6] + registers[0];
registers[7] = registers[10] + registers[0];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[12] << 1026;
[[wile(CONDITION){]]
registers[12] = registers[12] + registers[1];
memory[193] = registers[10];
registers[18]=registers[22] - registers[12]1;
if (registers[18] != 0 ) { PC = PC + -68; }
registers[6] = registers[6] + registers[4];
if (registers[9] <= 0 ) { PC = PC + 52; }
[[wile(CONDITION){]]
registers[10] = registers[0] + registers[0];
registers[8] = registers[6] + registers[0];
registers[7] = registers[10] + registers[0];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[12] << 1026;
[[wile(CONDITION){]]
registers[12] = registers[12] + registers[1];
memory[193] = registers[10];
registers[18]=registers[22] - registers[12]1;
if (registers[18] != 0 ) { PC = PC + -68; }
registers[6] = registers[6] + registers[4];
if (registers[9] <= 0 ) { PC = PC + 52; }
[[wile(CONDITION){]]
registers[10] = registers[0] + registers[0];
registers[8] = registers[6] + registers[0];
registers[7] = registers[10] + registers[0];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[12] << 1026;
[[wile(CONDITION){]]
registers[12] = registers[12] + registers[1];
memory[193] = registers[10];
registers[18]=registers[22] - registers[12]1;
if (registers[18] != 0 ) { PC = PC + -68; }
registers[6] = registers[6] + registers[4];
if (registers[9] <= 0 ) { PC = PC + 52; }
[[wile(CONDITION){]]
registers[10] = registers[0] + registers[0];
registers[8] = registers[6] + registers[0];
registers[7] = registers[10] + registers[0];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[12] << 1026;
[[wile(CONDITION){]]
registers[12] = registers[12] + registers[1];
memory[193] = registers[10];
registers[18]=registers[22] - registers[12]1;
if (registers[18] != 0 ) { PC = PC + -68; }
registers[6] = registers[6] + registers[4];
if (registers[9] <= 0 ) { PC = PC + 52; }
[[wile(CONDITION){]]
registers[10] = registers[0] + registers[0];
registers[8] = registers[6] + registers[0];
registers[7] = registers[10] + registers[0];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[12] << 1026;
[[wile(CONDITION){]]
registers[12] = registers[12] + registers[1];
memory[193] = registers[10];
registers[18]=registers[22] - registers[12]1;
if (registers[18] != 0 ) { PC = PC + -68; }
registers[6] = registers[6] + registers[4];
if (registers[9] <= 0 ) { PC = PC + 52; }
[[wile(CONDITION){]]
registers[10] = registers[0] + registers[0];
registers[8] = registers[6] + registers[0];
registers[7] = registers[10] + registers[0];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[12] << 1026;
[[wile(CONDITION){]]
registers[12] = registers[12] + registers[1];
memory[193] = registers[10];
registers[18]=registers[22] - registers[12]1;
if (registers[18] != 0 ) { PC = PC + -68; }
registers[6] = registers[6] + registers[4];
if (registers[9] <= 0 ) { PC = PC + 52; }
[[wile(CONDITION){]]
registers[10] = registers[0] + registers[0];
registers[8] = registers[6] + registers[0];
registers[7] = registers[10] + registers[0];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[12] << 1026;
[[wile(CONDITION){]]
registers[12] = registers[12] + registers[1];
memory[193] = registers[10];
registers[18]=registers[22] - registers[12]1;
if (registers[18] != 0 ) { PC = PC + -68; }
registers[6] = registers[6] + registers[4];
if (registers[9] <= 0 ) { PC = PC + 52; }
[[wile(CONDITION){]]
registers[10] = registers[0] + registers[0];
registers[8] = registers[6] + registers[0];
registers[7] = registers[10] + registers[0];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[12] << 1026;
[[wile(CONDITION){]]
registers[12] = registers[12] + registers[1];
memory[193] = registers[10];
registers[18]=registers[22] - registers[12]1;
if (registers[18] != 0 ) { PC = PC + -68; }
registers[6] = registers[6] + registers[4];
if (registers[9] <= 0 ) { PC = PC + 52; }
[[wile(CONDITION){]]
registers[10] = registers[0] + registers[0];
registers[8] = registers[6] + registers[0];
registers[7] = registers[10] + registers[0];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[12] << 1026;
[[wile(CONDITION){]]
registers[12] = registers[12] + registers[1];
memory[193] = registers[10];
registers[18]=registers[22] - registers[12]1;
if (registers[18] != 0 ) { PC = PC + -68; }
registers[6] = registers[6] + registers[4];
if (registers[9] <= 0 ) { PC = PC + 52; }
[[wile(CONDITION){]]
registers[10] = registers[0] + registers[0];
registers[8] = registers[6] + registers[0];
registers[7] = registers[10] + registers[0];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[12] << 1026;
[[wile(CONDITION){]]
registers[12] = registers[12] + registers[1];
memory[193] = registers[10];
registers[18]=registers[22] - registers[12]1;
if (registers[18] != 0 ) { PC = PC + -68; }
registers[6] = registers[6] + registers[4];
if (registers[9] <= 0 ) { PC = PC + 52; }
[[wile(CONDITION){]]
registers[10] = registers[0] + registers[0];
registers[8] = registers[6] + registers[0];
registers[7] = registers[10] + registers[0];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[12] << 1026;
[[wile(CONDITION){]]
registers[12] = registers[12] + registers[1];
memory[193] = registers[10];
registers[18]=registers[22] - registers[12]1;
if (registers[18] != 0 ) { PC = PC + -68; }
registers[6] = registers[6] + registers[4];
if (registers[9] <= 0 ) { PC = PC + 52; }
[[wile(CONDITION){]]
registers[10] = registers[0] + registers[0];
registers[8] = registers[6] + registers[0];
registers[7] = registers[10] + registers[0];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[12] << 1026;
[[wile(CONDITION){]]
registers[12] = registers[12] + registers[1];
memory[193] = registers[10];
registers[18]=registers[22] - registers[12]1;
if (registers[18] != 0 ) { PC = PC + -68; }
registers[6] = registers[6] + registers[4];
if (registers[9] <= 0 ) { PC = PC + 52; }
[[wile(CONDITION){]]
registers[10] = registers[0] + registers[0];
registers[8] = registers[6] + registers[0];
registers[7] = registers[10] + registers[0];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[12] << 1026;
[[wile(CONDITION){]]
registers[12] = registers[12] + registers[1];
memory[193] = registers[10];
registers[18]=registers[22] - registers[12]1;
if (registers[18] != 0 ) { PC = PC + -68; }
registers[6] = registers[6] + registers[4];
if (registers[9] <= 0 ) { PC = PC + 52; }
[[wile(CONDITION){]]
registers[10] = registers[0] + registers[0];
registers[8] = registers[6] + registers[0];
registers[7] = registers[10] + registers[0];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[12] << 1026;
[[wile(CONDITION){]]
registers[12] = registers[12] + registers[1];
memory[193] = registers[10];
registers[18]=registers[22] - registers[12]1;
if (registers[18] != 0 ) { PC = PC + -68; }
registers[6] = registers[6] + registers[4];
if (registers[9] <= 0 ) { PC = PC + 52; }
[[wile(CONDITION){]]
registers[10] = registers[0] + registers[0];
registers[8] = registers[6] + registers[0];
registers[7] = registers[10] + registers[0];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[12] << 1026;
[[wile(CONDITION){]]
registers[12] = registers[12] + registers[1];
memory[193] = registers[10];
registers[18]=registers[22] - registers[12]1;
if (registers[18] != 0 ) { PC = PC + -68; }
registers[6] = registers[6] + registers[4];
if (registers[9] <= 0 ) { PC = PC + 52; }
[[wile(CONDITION){]]
registers[10] = registers[0] + registers[0];
registers[8] = registers[6] + registers[0];
registers[7] = registers[10] + registers[0];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[12] << 1026;
[[wile(CONDITION){]]
registers[12] = registers[12] + registers[1];
memory[193] = registers[10];
registers[18]=registers[22] - registers[12]1;
if (registers[18] != 0 ) { PC = PC + -68; }
registers[6] = registers[6] + registers[4];
if (registers[9] <= 0 ) { PC = PC + 52; }
[[wile(CONDITION){]]
registers[10] = registers[0] + registers[0];
registers[8] = registers[6] + registers[0];
registers[7] = registers[10] + registers[0];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[12] << 1026;
[[wile(CONDITION){]]
registers[12] = registers[12] + registers[1];
memory[193] = registers[10];
registers[18]=registers[22] - registers[12]1;
if (registers[18] != 0 ) { PC = PC + -68; }
registers[6] = registers[6] + registers[4];
if (registers[9] <= 0 ) { PC = PC + 52; }
[[wile(CONDITION){]]
registers[10] = registers[0] + registers[0];
registers[8] = registers[6] + registers[0];
registers[7] = registers[10] + registers[0];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[12] << 1026;
[[wile(CONDITION){]]
registers[12] = registers[12] + registers[1];
memory[193] = registers[10];
registers[18]=registers[22] - registers[12]1;
if (registers[18] != 0 ) { PC = PC + -68; }
registers[6] = registers[6] + registers[4];
if (registers[9] <= 0 ) { PC = PC + 52; }
[[wile(CONDITION){]]
registers[10] = registers[0] + registers[0];
registers[8] = registers[6] + registers[0];
registers[7] = registers[10] + registers[0];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[12] << 1026;
[[wile(CONDITION){]]
registers[12] = registers[12] + registers[1];
memory[193] = registers[10];
registers[18]=registers[22] - registers[12]1;
if (registers[18] != 0 ) { PC = PC + -68; }
registers[6] = registers[6] + registers[4];
if (registers[9] <= 0 ) { PC = PC + 52; }
[[wile(CONDITION){]]
registers[10] = registers[0] + registers[0];
registers[8] = registers[6] + registers[0];
registers[7] = registers[10] + registers[0];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[12] << 1026;
[[wile(CONDITION){]]
registers[12] = registers[12] + registers[1];
memory[193] = registers[10];
registers[18]=registers[22] - registers[12]1;
if (registers[18] != 0 ) { PC = PC + -68; }
registers[6] = registers[6] + registers[4];
if (registers[9] <= 0 ) { PC = PC + 52; }
[[wile(CONDITION){]]
registers[10] = registers[0] + registers[0];
registers[8] = registers[6] + registers[0];
registers[7] = registers[10] + registers[0];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[12] << 1026;
[[wile(CONDITION){]]
registers[12] = registers[12] + registers[1];
memory[193] = registers[10];
registers[18]=registers[22] - registers[12]1;
if (registers[18] != 0 ) { PC = PC + -68; }
registers[6] = registers[6] + registers[4];
if (registers[9] <= 0 ) { PC = PC + 52; }
[[wile(CONDITION){]]
registers[10] = registers[0] + registers[0];
registers[8] = registers[6] + registers[0];
registers[7] = registers[10] + registers[0];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[12] << 1026;
[[wile(CONDITION){]]
registers[12] = registers[12] + registers[1];
memory[193] = registers[10];
registers[18]=registers[22] - registers[12]1;
if (registers[18] != 0 ) { PC = PC + -68; }
registers[6] = registers[6] + registers[4];
if (registers[9] <= 0 ) { PC = PC + 52; }
[[wile(CONDITION){]]
registers[10] = registers[0] + registers[0];
registers[8] = registers[6] + registers[0];
registers[7] = registers[10] + registers[0];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[12] << 1026;
[[wile(CONDITION){]]
registers[12] = registers[12] + registers[1];
memory[193] = registers[10];
registers[18]=registers[22] - registers[12]1;
if (registers[18] != 0 ) { PC = PC + -68; }
registers[6] = registers[6] + registers[4];
if (registers[9] <= 0 ) { PC = PC + 52; }
[[wile(CONDITION){]]
registers[10] = registers[0] + registers[0];
registers[8] = registers[6] + registers[0];
registers[7] = registers[10] + registers[0];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[12] << 1026;
[[wile(CONDITION){]]
registers[12] = registers[12] + registers[1];
memory[193] = registers[10];
registers[18]=registers[22] - registers[12]1;
if (registers[18] != 0 ) { PC = PC + -68; }
registers[6] = registers[6] + registers[4];
if (registers[9] <= 0 ) { PC = PC + 52; }
[[wile(CONDITION){]]
registers[10] = registers[0] + registers[0];
registers[8] = registers[6] + registers[0];
registers[7] = registers[10] + registers[0];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[12] << 1026;
[[wile(CONDITION){]]
registers[12] = registers[12] + registers[1];
memory[193] = registers[10];
registers[18]=registers[22] - registers[12]1;
if (registers[18] != 0 ) { PC = PC + -68; }
registers[6] = registers[6] + registers[4];
if (registers[9] <= 0 ) { PC = PC + 52; }
[[wile(CONDITION){]]
registers[10] = registers[0] + registers[0];
registers[8] = registers[6] + registers[0];
registers[7] = registers[10] + registers[0];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[12] << 1026;
[[wile(CONDITION){]]
registers[12] = registers[12] + registers[1];
memory[193] = registers[10];
registers[18]=registers[22] - registers[12]1;
if (registers[18] != 0 ) { PC = PC + -68; }
registers[6] = registers[6] + registers[4];
if (registers[9] <= 0 ) { PC = PC + 52; }
[[wile(CONDITION){]]
registers[10] = registers[0] + registers[0];
registers[8] = registers[6] + registers[0];
registers[7] = registers[10] + registers[0];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[12] << 1026;
[[wile(CONDITION){]]
registers[12] = registers[12] + registers[1];
memory[193] = registers[10];
registers[18]=registers[22] - registers[12]1;
if (registers[18] != 0 ) { PC = PC + -68; }
registers[6] = registers[6] + registers[4];
if (registers[9] <= 0 ) { PC = PC + 52; }
[[wile(CONDITION){]]
registers[10] = registers[0] + registers[0];
registers[8] = registers[6] + registers[0];
registers[7] = registers[10] + registers[0];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[12] << 1026;
[[wile(CONDITION){]]
registers[12] = registers[12] + registers[1];
memory[193] = registers[10];
registers[18]=registers[22] - registers[12]1;
if (registers[18] != 0 ) { PC = PC + -68; }
registers[6] = registers[6] + registers[4];
if (registers[9] <= 0 ) { PC = PC + 52; }
[[wile(CONDITION){]]
registers[10] = registers[0] + registers[0];
registers[8] = registers[6] + registers[0];
registers[7] = registers[10] + registers[0];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[12] << 1026;
[[wile(CONDITION){]]
registers[12] = registers[12] + registers[1];
memory[193] = registers[10];
registers[18]=registers[22] - registers[12]1;
if (registers[18] != 0 ) { PC = PC + -68; }
registers[6] = registers[6] + registers[4];
if (registers[9] <= 0 ) { PC = PC + 52; }
[[wile(CONDITION){]]
registers[10] = registers[0] + registers[0];
registers[8] = registers[6] + registers[0];
registers[7] = registers[10] + registers[0];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[12] << 1026;
[[wile(CONDITION){]]
registers[12] = registers[12] + registers[1];
memory[193] = registers[10];
registers[18]=registers[22] - registers[12]1;
if (registers[18] != 0 ) { PC = PC + -68; }
registers[6] = registers[6] + registers[4];
if (registers[9] <= 0 ) { PC = PC + 52; }
[[wile(CONDITION){]]
registers[10] = registers[0] + registers[0];
registers[8] = registers[6] + registers[0];
registers[7] = registers[10] + registers[0];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[12] << 1026;
[[wile(CONDITION){]]
registers[12] = registers[12] + registers[1];
memory[193] = registers[10];
registers[18]=registers[22] - registers[12]1;
if (registers[18] != 0 ) { PC = PC + -68; }
registers[6] = registers[6] + registers[4];
if (registers[9] <= 0 ) { PC = PC + 52; }
[[wile(CONDITION){]]
registers[10] = registers[0] + registers[0];
registers[8] = registers[6] + registers[0];
registers[7] = registers[10] + registers[0];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[12] << 1026;
[[wile(CONDITION){]]
registers[12] = registers[12] + registers[1];
memory[193] = registers[10];
registers[18]=registers[22] - registers[12]1;
if (registers[18] != 0 ) { PC = PC + -68; }
registers[6] = registers[6] + registers[4];
if (registers[9] <= 0 ) { PC = PC + 52; }
[[wile(CONDITION){]]
registers[10] = registers[0] + registers[0];
registers[8] = registers[6] + registers[0];
registers[7] = registers[10] + registers[0];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[12] << 1026;
[[wile(CONDITION){]]
registers[12] = registers[12] + registers[1];
memory[193] = registers[10];
registers[18]=registers[22] - registers[12]1;
if (registers[18] != 0 ) { PC = PC + -68; }
registers[6] = registers[6] + registers[4];
if (registers[9] <= 0 ) { PC = PC + 52; }
[[wile(CONDITION){]]
registers[10] = registers[0] + registers[0];
registers[8] = registers[6] + registers[0];
registers[7] = registers[10] + registers[0];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[12] << 1026;
[[wile(CONDITION){]]
registers[12] = registers[12] + registers[1];
memory[193] = registers[10];
registers[18]=registers[22] - registers[12]1;
if (registers[18] != 0 ) { PC = PC + -68; }
registers[6] = registers[6] + registers[4];
if (registers[9] <= 0 ) { PC = PC + 52; }
[[wile(CONDITION){]]
registers[10] = registers[0] + registers[0];
registers[8] = registers[6] + registers[0];
registers[7] = registers[10] + registers[0];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[12] << 1026;
[[wile(CONDITION){]]
registers[12] = registers[12] + registers[1];
memory[193] = registers[10];
registers[18]=registers[22] - registers[12]1;
if (registers[18] != 0 ) { PC = PC + -68; }
registers[6] = registers[6] + registers[4];
if (registers[9] <= 0 ) { PC = PC + 52; }
[[wile(CONDITION){]]
registers[10] = registers[0] + registers[0];
registers[8] = registers[6] + registers[0];
registers[7] = registers[10] + registers[0];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[12] << 1026;
[[wile(CONDITION){]]
registers[12] = registers[12] + registers[1];
memory[193] = registers[10];
registers[18]=registers[22] - registers[12]1;
if (registers[18] != 0 ) { PC = PC + -68; }
registers[6] = registers[6] + registers[4];
if (registers[9] <= 0 ) { PC = PC + 52; }
[[wile(CONDITION){]]
registers[10] = registers[0] + registers[0];
registers[8] = registers[6] + registers[0];
registers[7] = registers[10] + registers[0];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[12] << 1026;
[[wile(CONDITION){]]
registers[12] = registers[12] + registers[1];
memory[193] = registers[10];
registers[18]=registers[22] - registers[12]1;
if (registers[18] != 0 ) { PC = PC + -68; }
registers[6] = registers[6] + registers[4];
if (registers[9] <= 0 ) { PC = PC + 52; }
[[wile(CONDITION){]]
registers[10] = registers[0] + registers[0];
registers[8] = registers[6] + registers[0];
registers[7] = registers[10] + registers[0];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[12] << 1026;
[[wile(CONDITION){]]
registers[12] = registers[12] + registers[1];
memory[193] = registers[10];
registers[18]=registers[22] - registers[12]1;
if (registers[18] != 0 ) { PC = PC + -68; }
registers[6] = registers[6] + registers[4];
if (registers[9] <= 0 ) { PC = PC + 52; }
[[wile(CONDITION){]]
registers[10] = registers[0] + registers[0];
registers[8] = registers[6] + registers[0];
registers[7] = registers[10] + registers[0];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[12] << 1026;
[[wile(CONDITION){]]
registers[12] = registers[12] + registers[1];
memory[193] = registers[10];
registers[18]=registers[22] - registers[12]1;
if (registers[18] != 0 ) { PC = PC + -68; }
registers[6] = registers[6] + registers[4];
if (registers[9] <= 0 ) { PC = PC + 52; }
[[wile(CONDITION){]]
registers[10] = registers[0] + registers[0];
registers[8] = registers[6] + registers[0];
registers[7] = registers[10] + registers[0];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[12] << 1026;
[[wile(CONDITION){]]
registers[12] = registers[12] + registers[1];
memory[193] = registers[10];
registers[18]=registers[22] - registers[12]1;
if (registers[18] != 0 ) { PC = PC + -68; }
registers[6] = registers[6] + registers[4];
if (registers[9] <= 0 ) { PC = PC + 52; }
[[wile(CONDITION){]]
registers[10] = registers[0] + registers[0];
registers[8] = registers[6] + registers[0];
registers[7] = registers[10] + registers[0];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[12] << 1026;
[[wile(CONDITION){]]
registers[12] = registers[12] + registers[1];
memory[193] = registers[10];
registers[18]=registers[22] - registers[12]1;
if (registers[18] != 0 ) { PC = PC + -68; }
registers[6] = registers[6] + registers[4];
if (registers[9] <= 0 ) { PC = PC + 52; }
[[wile(CONDITION){]]
registers[10] = registers[0] + registers[0];
registers[8] = registers[6] + registers[0];
registers[7] = registers[10] + registers[0];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[12] << 1026;
[[wile(CONDITION){]]
registers[12] = registers[12] + registers[1];
memory[193] = registers[10];
registers[18]=registers[22] - registers[12]1;
if (registers[18] != 0 ) { PC = PC + -68; }
registers[6] = registers[6] + registers[4];
if (registers[9] <= 0 ) { PC = PC + 52; }
[[wile(CONDITION){]]
registers[10] = registers[0] + registers[0];
registers[8] = registers[6] + registers[0];
registers[7] = registers[10] + registers[0];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[12] << 1026;
[[wile(CONDITION){]]
registers[12] = registers[12] + registers[1];
memory[193] = registers[10];
registers[18]=registers[22] - registers[12]1;
if (registers[18] != 0 ) { PC = PC + -68; }
registers[6] = registers[6] + registers[4];
if (registers[9] <= 0 ) { PC = PC + 52; }
[[wile(CONDITION){]]
registers[10] = registers[0] + registers[0];
registers[8] = registers[6] + registers[0];
registers[7] = registers[10] + registers[0];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[12] << 1026;
[[wile(CONDITION){]]
registers[12] = registers[12] + registers[1];
memory[193] = registers[10];
registers[18]=registers[22] - registers[12]1;
if (registers[18] != 0 ) { PC = PC + -68; }
registers[6] = registers[6] + registers[4];
if (registers[9] <= 0 ) { PC = PC + 52; }
[[wile(CONDITION){]]
registers[10] = registers[0] + registers[0];
registers[8] = registers[6] + registers[0];
registers[7] = registers[10] + registers[0];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[12] << 1026;
[[wile(CONDITION){]]
registers[12] = registers[12] + registers[1];
memory[193] = registers[10];
registers[18]=registers[22] - registers[12]1;
if (registers[18] != 0 ) { PC = PC + -68; }
registers[6] = registers[6] + registers[4];
if (registers[9] <= 0 ) { PC = PC + 52; }
[[wile(CONDITION){]]
registers[10] = registers[0] + registers[0];
registers[8] = registers[6] + registers[0];
registers[7] = registers[10] + registers[0];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[12] << 1026;
[[wile(CONDITION){]]
registers[12] = registers[12] + registers[1];
memory[193] = registers[10];
registers[18]=registers[22] - registers[12]1;
if (registers[18] != 0 ) { PC = PC + -68; }
registers[6] = registers[6] + registers[4];
if (registers[9] <= 0 ) { PC = PC + 52; }
[[wile(CONDITION){]]
registers[10] = registers[0] + registers[0];
registers[8] = registers[6] + registers[0];
registers[7] = registers[10] + registers[0];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[12] << 1026;
[[wile(CONDITION){]]
registers[12] = registers[12] + registers[1];
memory[193] = registers[10];
registers[18]=registers[22] - registers[12]1;
if (registers[18] != 0 ) { PC = PC + -68; }
registers[6] = registers[6] + registers[4];
if (registers[9] <= 0 ) { PC = PC + 52; }
[[wile(CONDITION){]]
registers[10] = registers[0] + registers[0];
registers[8] = registers[6] + registers[0];
registers[7] = registers[10] + registers[0];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[12] << 1026;
[[wile(CONDITION){]]
registers[12] = registers[12] + registers[1];
memory[193] = registers[10];
registers[18]=registers[22] - registers[12]1;
if (registers[18] != 0 ) { PC = PC + -68; }
registers[6] = registers[6] + registers[4];
if (registers[9] <= 0 ) { PC = PC + 52; }
[[wile(CONDITION){]]
registers[10] = registers[0] + registers[0];
registers[8] = registers[6] + registers[0];
registers[7] = registers[10] + registers[0];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[12] << 1026;
[[wile(CONDITION){]]
registers[12] = registers[12] + registers[1];
memory[193] = registers[10];
registers[18]=registers[22] - registers[12]1;
if (registers[18] != 0 ) { PC = PC + -68; }
registers[6] = registers[6] + registers[4];
if (registers[9] <= 0 ) { PC = PC + 52; }
[[wile(CONDITION){]]
registers[10] = registers[0] + registers[0];
registers[8] = registers[6] + registers[0];
registers[7] = registers[10] + registers[0];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[12] << 1026;
[[wile(CONDITION){]]
registers[12] = registers[12] + registers[1];
memory[193] = registers[10];
registers[18]=registers[22] - registers[12]1;
if (registers[18] != 0 ) { PC = PC + -68; }
registers[6] = registers[6] + registers[4];
if (registers[9] <= 0 ) { PC = PC + 52; }
[[wile(CONDITION){]]
registers[10] = registers[0] + registers[0];
registers[8] = registers[6] + registers[0];
registers[7] = registers[10] + registers[0];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[12] << 1026;
[[wile(CONDITION){]]
registers[12] = registers[12] + registers[1];
memory[193] = registers[10];
registers[18]=registers[22] - registers[12]1;
if (registers[18] != 0 ) { PC = PC + -68; }
registers[6] = registers[6] + registers[4];
if (registers[9] <= 0 ) { PC = PC + 52; }
[[wile(CONDITION){]]
registers[10] = registers[0] + registers[0];
registers[8] = registers[6] + registers[0];
registers[7] = registers[10] + registers[0];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[12] << 1026;
[[wile(CONDITION){]]
registers[12] = registers[12] + registers[1];
memory[193] = registers[10];
registers[18]=registers[22] - registers[12]1;
if (registers[18] != 0 ) { PC = PC + -68; }
registers[6] = registers[6] + registers[4];
if (registers[9] <= 0 ) { PC = PC + 52; }
[[wile(CONDITION){]]
registers[10] = registers[0] + registers[0];
registers[8] = registers[6] + registers[0];
registers[7] = registers[10] + registers[0];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[12] << 1026;
[[wile(CONDITION){]]
registers[12] = registers[12] + registers[1];
memory[193] = registers[10];
registers[18]=registers[22] - registers[12]1;
if (registers[18] != 0 ) { PC = PC + -68; }
registers[6] = registers[6] + registers[4];
if (registers[9] <= 0 ) { PC = PC + 52; }
[[wile(CONDITION){]]
registers[10] = registers[0] + registers[0];
registers[8] = registers[6] + registers[0];
registers[7] = registers[10] + registers[0];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[12] << 1026;
[[wile(CONDITION){]]
registers[12] = registers[12] + registers[1];
memory[193] = registers[10];
registers[18]=registers[22] - registers[12]1;
if (registers[18] != 0 ) { PC = PC + -68; }
registers[6] = registers[6] + registers[4];
if (registers[9] <= 0 ) { PC = PC + 52; }
[[wile(CONDITION){]]
registers[10] = registers[0] + registers[0];
registers[8] = registers[6] + registers[0];
registers[7] = registers[10] + registers[0];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[12] << 1026;
[[wile(CONDITION){]]
registers[12] = registers[12] + registers[1];
memory[193] = registers[10];
registers[18]=registers[22] - registers[12]1;
if (registers[18] != 0 ) { PC = PC + -68; }
registers[6] = registers[6] + registers[4];
if (registers[9] <= 0 ) { PC = PC + 52; }
[[wile(CONDITION){]]
registers[10] = registers[0] + registers[0];
registers[8] = registers[6] + registers[0];
registers[7] = registers[10] + registers[0];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[12] << 1026;
[[wile(CONDITION){]]
registers[12] = registers[12] + registers[1];
memory[193] = registers[10];
registers[18]=registers[22] - registers[12]1;
if (registers[18] != 0 ) { PC = PC + -68; }
registers[6] = registers[6] + registers[4];
if (registers[9] <= 0 ) { PC = PC + 52; }
[[wile(CONDITION){]]
registers[10] = registers[0] + registers[0];
registers[8] = registers[6] + registers[0];
registers[7] = registers[10] + registers[0];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[12] << 1026;
[[wile(CONDITION){]]
registers[12] = registers[12] + registers[1];
memory[193] = registers[10];
registers[18]=registers[22] - registers[12]1;
if (registers[18] != 0 ) { PC = PC + -68; }
registers[6] = registers[6] + registers[4];
if (registers[9] <= 0 ) { PC = PC + 52; }
[[wile(CONDITION){]]
registers[10] = registers[0] + registers[0];
registers[8] = registers[6] + registers[0];
registers[7] = registers[10] + registers[0];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[12] << 1026;
[[wile(CONDITION){]]
registers[12] = registers[12] + registers[1];
memory[193] = registers[10];
registers[18]=registers[22] - registers[12]1;
if (registers[18] != 0 ) { PC = PC + -68; }
registers[6] = registers[6] + registers[4];
if (registers[9] <= 0 ) { PC = PC + 52; }
[[wile(CONDITION){]]
registers[10] = registers[0] + registers[0];
registers[8] = registers[6] + registers[0];
registers[7] = registers[10] + registers[0];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[12] << 1026;
[[wile(CONDITION){]]
registers[12] = registers[12] + registers[1];
memory[193] = registers[10];
registers[18]=registers[22] - registers[12]1;
if (registers[18] != 0 ) { PC = PC + -68; }
registers[6] = registers[6] + registers[4];
if (registers[9] <= 0 ) { PC = PC + 52; }
[[wile(CONDITION){]]
registers[10] = registers[0] + registers[0];
registers[8] = registers[6] + registers[0];
registers[7] = registers[10] + registers[0];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[12] << 1026;
[[wile(CONDITION){]]
registers[12] = registers[12] + registers[1];
memory[193] = registers[10];
registers[18]=registers[22] - registers[12]1;
if (registers[18] != 0 ) { PC = PC + -68; }
registers[6] = registers[6] + registers[4];
if (registers[9] <= 0 ) { PC = PC + 52; }
[[wile(CONDITION){]]
registers[10] = registers[0] + registers[0];
registers[8] = registers[6] + registers[0];
registers[7] = registers[10] + registers[0];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[12] << 1026;
[[wile(CONDITION){]]
registers[12] = registers[12] + registers[1];
memory[193] = registers[10];
registers[18]=registers[22] - registers[12]1;
if (registers[18] != 0 ) { PC = PC + -68; }
registers[6] = registers[6] + registers[4];
if (registers[9] <= 0 ) { PC = PC + 52; }
[[wile(CONDITION){]]
registers[10] = registers[0] + registers[0];
registers[8] = registers[6] + registers[0];
registers[7] = registers[10] + registers[0];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[12] << 1026;
[[wile(CONDITION){]]
registers[12] = registers[12] + registers[1];
memory[193] = registers[10];
registers[18]=registers[22] - registers[12]1;
if (registers[18] != 0 ) { PC = PC + -68; }
registers[6] = registers[6] + registers[4];
if (registers[9] <= 0 ) { PC = PC + 52; }
[[wile(CONDITION){]]
registers[10] = registers[0] + registers[0];
registers[8] = registers[6] + registers[0];
registers[7] = registers[10] + registers[0];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[12] << 1026;
[[wile(CONDITION){]]
registers[12] = registers[12] + registers[1];
memory[193] = registers[10];
registers[18]=registers[22] - registers[12]1;
if (registers[18] != 0 ) { PC = PC + -68; }
registers[6] = registers[6] + registers[4];
if (registers[9] <= 0 ) { PC = PC + 52; }
[[wile(CONDITION){]]
registers[10] = registers[0] + registers[0];
registers[8] = registers[6] + registers[0];
registers[7] = registers[10] + registers[0];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[12] << 1026;
[[wile(CONDITION){]]
registers[12] = registers[12] + registers[1];
memory[193] = registers[10];
registers[18]=registers[22] - registers[12]1;
if (registers[18] != 0 ) { PC = PC + -68; }
registers[6] = registers[6] + registers[4];
if (registers[9] <= 0 ) { PC = PC + 52; }
[[wile(CONDITION){]]
registers[10] = registers[0] + registers[0];
registers[8] = registers[6] + registers[0];
registers[7] = registers[10] + registers[0];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[12] << 1026;
[[wile(CONDITION){]]
registers[12] = registers[12] + registers[1];
memory[193] = registers[10];
registers[18]=registers[22] - registers[12]1;
if (registers[18] != 0 ) { PC = PC + -68; }
registers[6] = registers[6] + registers[4];
if (registers[9] <= 0 ) { PC = PC + 52; }
[[wile(CONDITION){]]
registers[10] = registers[0] + registers[0];
registers[8] = registers[6] + registers[0];
registers[7] = registers[10] + registers[0];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[7] << 1026;
[[wile(CONDITION){]]
registers[5] = memory[80];
registers[4] = memory[113];
registers[7] = registers[7] + registers[1];
registers[8] = registers[8] + registers[-4];
registers[4] = registers[4] * registers[5];
registers[18]=registers[9] - registers[7]1;
if (registers[18] != 0 ) { PC = PC + -28; }
registers[10] = registers[10] + registers[4];
registers[3]= ( registers[12] << 1026;
[[wile(CONDITION){]]
registers[12] = registers[12] + registers[1];
memory[193] = registers[10];
registers[18]=registers[22] - registers[12]1;
if (registers[18] != 0 ) { PC = PC + -68; }
registers[6] = registers[6] + registers[4];
registers[19] = memory[14];
registers[22] = memory[18];
//[ERROR]: Couldn't understand instruction
registers[1] = registers[1] + registers[12];
registers[15] = memory[10];
registers[3] = registers[0] + registers[0];
//[ERROR]: Couldn't understand instruction
registers[1] = registers[1] + registers[1052];
registers[19] = registers[3] + registers[0];
PC = PC + 832;
registers[15] = PC;
registers[0]=registers[0]||registers[0];
registers[1] = registers[1] + registers[-8];
}
